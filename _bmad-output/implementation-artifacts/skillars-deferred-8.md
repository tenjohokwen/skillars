...
- [x] [Review][Fixed] `resolveParentName()` can render "null null" when a user's first/last name is null [BookingService.java:634] — fixed: builds the name from non-null parts, trims, and falls back to `"Unknown Parent"` if both are blank, instead of naive string concatenation.
- [x] [Review][Fixed] `declineBooking_wrongCoach_returns403` reads `createResp.getBody().get("id")` without asserting creation succeeded first [BookingRequestResourceIT.java:~525] — fixed: added `assertThat(createResp.getStatusCode()).isEqualTo(HttpStatus.CREATED)` before reading `id`; also applied the same fix to `acceptBooking_wrongCoach_returns403` since it had the identical gap.
...
- [x] [Review][Dismiss] Hardcoded `PLAYER_ID = 9360000001L` reused against the shared `SecurityIT.SEC_DATA_SQL_PATH` fixture [NeglectedSkillDetectionServiceIT.java:47] — investigated and confirmed non-issue: `secData.sql` only inserts into `main.sec` (unrelated to `player_id`), and a repo-wide grep confirms `9360000001L` is not reused as a `PLAYER_ID` by any other test/IT class.
...
- 2026-07-02: Resolved 3 deferred items (D1, D2, D3). D1: `resolveParentName()` now builds the name from non-null parts and falls back to `"Unknown Parent"` instead of rendering `"null null"`. D2: added a `HttpStatus.CREATED` assertion before reading the created booking's `id` in both `declineBooking_wrongCoach_returns403` and `acceptBooking_wrongCoach_returns403` (same pre-existing gap in both). D3: investigated and confirmed non-issue — no code change needed. `BookingRequestResourceIT` re-verified (2/2, then 2/2 for the sort/parentName tests), `mvn -o compile` and `test-compile` both clean.
