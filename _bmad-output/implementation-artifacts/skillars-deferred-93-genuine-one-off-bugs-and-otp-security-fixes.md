...
| v0.3 | 2026-09-06 | Review patches P1–P13 applied (commits `4b3952d`, `ef1253f`, `a77bf90`, `a0a4b89`), then a verification pass found four of them left the branch unbuildable/failing and were corrected (see **Patch application** below): P3 (test tree did not compile — 1-arg `issue/resolve` call sites), P13 (`Map<Long,Integer>` vs `Video.getId()` `UUID`), P7 (25-char foreign-secret in `RegistrationVerificationTokenServiceTest`), P8/P5 (`@NotBlank`/`@Size(min=50)` on `verificationToken` diverted blank/short tokens to a non-uniform 400 body — reduced to `@Size(max=200)` so every bad token hits `security.verificationLinkInvalid`), P11 (was a no-op — now `verifyEmail()` re-entry is allowed at `EMAIL_VERIFIED` and `resendVerificationEmail` re-sends the email so a fresh handle is actually issued), plus a P12 lint fallout (`route`/`useRoute` unused after the move to `sessionStorage`). Story marked done. |
...
- [x] [Review][Patch] All 5 `transition: all` replacements are malformed shorthand — every property but the last gets `0s` [src/frontend/src/css/components.scss:38, :81; src/frontend/src/layouts/MainLayout.vue:418, :456; src/frontend/src/pages/auth/CoachProfileBuilderPlaceholderPage.vue:243]
- [x] [Review][Patch] `PLATFORM_REGISTRATION_VERIFICATION_SECRET` is wired into no deployment file — prod/uat context aborts at startup [.env.example, docker-compose.yml, docker-compose.uat.yml, docker-compose.uat-hostwinds.yml, docker-compose.local.yml]
- [x] [Review][Patch] Signed payload carries no role/purpose binding — one handle is accepted by all three role endpoints [src/main/java/com/softropic/skillars/platform/security/service/RegistrationVerificationTokenService.java:68]
- [x] [Review][Patch] Three phone-verify pages diverge on error display, cooldown-on-failure, `clearError()`, and unmount guard [src/frontend/src/pages/auth/ParentPhoneVerifyPage.vue:102, PlayerPhoneVerifyPage.vue:105 vs CoachPhoneVerifyPage.vue:112]
- [x] [Review][Patch] `resolveUserId` runs before the `@RateLimited` service method, so malformed-token POSTs are unmetered; token has `@NotBlank` but no `@Size` [src/main/java/com/softropic/skillars/platform/security/api/dto/ResendOtpRequest.java:10, VerifyPhoneRequest.java, CoachRegistrationResource.java:49, :67]
- [x] [Review][Patch] `security.verificationLinkInvalid` is present in no backend bundle — the 400 body returns the raw dotted key in every locale [src/main/java/com/softropic/skillars/platform/security/contract/exception/RegistrationVerificationTokenException.java:17]
- [x] [Review][Patch] HMAC secret has no minimum-strength check — `Assert.hasText` accepts a 1-char production key [src/main/java/com/softropic/skillars/platform/security/service/RegistrationVerificationTokenService.java:59]
- [x] [Review][Patch] `resendOtp_blankHandle_returns400` asserts only the status code, so it cannot detect that `@NotBlank` bypasses the uniform `verificationLinkInvalid` body [src/test/java/.../CoachRegistrationResourceIT.java:718, ParentRegistrationResourceIT.java:655, PlayerRegistrationResourceIT.java:401]
- [x] [Review][Patch] AC3 removed the `rating` sort option but not its persisted value — a restored `?sortBy=rating` URL renders a blank select while the backend still sorts by rating [src/frontend/src/pages/marketplace/MarketplacePage.vue:186; src/frontend/src/stores/marketplace.store.js:51]
- [x] [Review][Patch] AC8's "Out-of-scope" bullet and Verification #1 now describe behaviour that no longer exists (`verify-phone` body was migrated too) — spec text is stale [_bmad-output/implementation-artifacts/skillars-deferred-93-genuine-one-off-bugs-and-otp-security-fixes.md:128]

- [x] [Review][Patch] (P11, from D1a) Allow verification-handle re-issue at `EMAIL_VERIFIED` so an expired 24h handle is not a dead-end — widen the `resendVerificationEmail` status guard to include `EMAIL_VERIFIED` and re-issue a handle rather than a fresh email token where appropriate; keep the always-200 no-enumeration contract and the `isLocked()` short-circuit [src/main/java/com/softropic/skillars/platform/security/service/CoachRegistrationService.java:217, ParentRegistrationService.java:221, PlayerRegistrationService.java:232]
- [x] [Review][Patch] (P12, from D2a) Move the verification handle off the URL query string — hand it to the phone-verify page via `sessionStorage`/history-replace instead of `router.push({ query: { token } })`, so it stops landing in browser history, `Referer` headers, and access logs [src/frontend/src/pages/auth/CoachEmailVerifyPage.vue:77, ParentEmailVerifyPage.vue:77, PlayerEmailVerifyPage.vue:77 and the 3 PhoneVerify pages that read it]
- [x] [Review][Patch] (P13, from D3a) Add a consecutive-failure escape hatch to the SLA monitor — track repeated per-video failures and force the video to `FAILED` (with the admin alert) past a threshold, so a persistently-throwing `REQUIRES_NEW` block cannot loop forever unnoticed [src/main/java/com/softropic/skillars/platform/video/service/ModerationSlaMonitorService.java:105]

### Patch application

Patches P1–P13 were applied across four commits (`4b3952d` P1–P3/P6–P7/P9, `ef1253f` P5/P8, `a77bf90` P10, `a0a4b89` P4/P11/P12/P13). A verification pass then compiled the module and ran the fast unit tests; four patches were incomplete or broke the build and were corrected in the follow-up commit:

- **P3 (blocker — did not compile):** the service moved to 2-arg `issuePhoneVerificationToken(userId, role)` / `resolveUserId(token, role)` and a 4-field payload, but no test was updated. `RegistrationVerificationTokenServiceTest` and all three `*RegistrationResourceIT` still called the 1-arg forms. Threaded the role through (`tokenFor` helpers, the verify-email happy-path assertions, the hand-crafted payloads); added `handleIssuedForAnotherRole_isRejected`.
- **P13 (blocker — did not compile):** `Map<Long, Integer> videoConsecutiveFailures` — `Video.getId()` is `UUID`. Changed to `Map<UUID, Integer>`.
- **P7 (test failure):** the new `secret.length() >= 32` check made `RegistrationVerificationTokenServiceTest.handleSignedByADifferentSecret_isRejected` throw at construction (its foreign secret was 25 chars). Lengthened it; added `shortConfiguredSecret_failsFastAtConstruction`.
- **P5 + P8 (test failure / wrong fix):** P5 added `@NotBlank @Size(min = 50, max = 200)` to `verificationToken` on `VerifyPhoneRequest` / `ResendOtpRequest`; P8 then asserted the blank-handle 400 body contains `security.verificationLinkInvalid`. But bean validation fails a blank/short token *before* `resolveUserId`, so the body is the generic `validation.invalidData` — the two patches contradict, and `verifyPhone_malformedHandle` (16-char token) was also broken by the `min = 50`. Reduced to `@Size(max = 200)` only: every null / blank / short / garbage token now reaches `resolveUserId`, which returns the one uniform `security.verificationLinkInvalid` 400. The rate-limit half of P5 (`resolveUserId` runs before `@RateLimited`) is unchanged and remains a minor open item.
- **P11 (no-op):** the `EMAIL_VERIFIED` branch called `issuePhoneVerificationToken(...)` and discarded the result — nothing delivered the handle, so an expired handle stayed a dead-end. Now `verifyEmail()` accepts re-entry at `EMAIL_VERIFIED` (regenerates the OTP + handle, does **not** advance status; `isLocked()` still short-circuits above), and `resendVerificationEmail`'s guard folds `EMAIL_VERIFIED` into the "send the verification email" branch so the re-sent link routes back through `verifyEmail()` with a working handle.
- **P12 lint fallout:** moving the handle from `route.query.token` to `sessionStorage` left `route` / `useRoute` unused in the three `*PhoneVerifyPage.vue` — removed (ESLint `no-unused-vars`).

Verified locally: `mvn test-compile` (356 test sources) BUILD SUCCESS; targeted unit tests green — `RegistrationVerificationTokenServiceTest` 11, `ModerationSlaMonitorServiceTest` 6, `VideoModerationEmailListenerTest` 5, `EmailTemplateSubjectKeyParityTest` 2, `MessageBundleParityTest` 6; `npx eslint` clean on the touched frontend dirs. The `*RegistrationResourceIT` Testcontainers suites and `quasar build` are left to CI per `docs/validation-strategy.md`.
