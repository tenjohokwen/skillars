...
- [x] [Review][Defer] `AccountManagementFacade.toUser()` null-guard on `getEmail()` — **Resolved (verified, no code change):** confirmed `persistUser()`/`toUser()` are private and reachable only via the public `registerAccount()`, which has exactly one caller codebase-wide — `AccountResource.registerAccount(@Valid @RequestBody UserDto)` (`AccountResource.java:67`). `UserDto.email` is `@NotNull` (`UserDto.java:44`), enforced by `@Valid` at that sole entry point. No traced or untraced internal caller can reach `toUser()` with a null email. The guard is harmless dead defensive code; left in place as-is, no follow-up needed.
- [x] [Review][Defer] GDPR Article 17 erasure (`MessageRepository.deleteAllBySenderId`, unchanged by this diff) still hard-deletes messages regardless of open abuse reports, while the new AC1 logic now explicitly protects them from the retention sweep — a policy asymmetry on the same table introduced as a side effect of this diff. **Resolved (product decision, 2026-07-03):** erasure remains unconditional — Article 17's legal deadline takes priority over an open abuse-report hold. `deleteAllBySenderId` is intentionally left as-is; no code change. Accepted tradeoff: if erasure runs while an investigation is open, that message's evidence is lost. No further action.
...
- 2026-07-03 — Closed both deferred items, no code changes. `AccountManagementFacade.toUser()` null-guard: verified via grep/trace that the only path to `toUser()` is `AccountResource.registerAccount()` with `@Valid` + `@NotNull UserDto.email`, so no caller can hit the null case — confirmed harmless, left as-is. GDPR erasure vs. abuse-report hold: product decision made — `deleteAllBySenderId` stays unconditional; Article 17 erasure takes priority over an open investigation hold, accepting that evidence on erased users' reported messages is lost if erasure runs mid-investigation.
