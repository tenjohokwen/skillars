...
| 2026-09-09 | **CI round-trip on PR #161 — 4 fix commits.** (1) `SessionPackForfeitureSchedulerTest`: `mock(User.class)` + `lenient().when(getEmail())` threw `UnfinishedStubbingException` ×4 → switched to `new User()`/`setEmail()` (codebase convention). (2) `payment.api.js`: prettier printWidth wrap of `fetchMyStrikes`. (3) `SessionPauseResumeServiceTest`: stale `.hasMessageContaining("concurrently")` → `"concurrent modification"` (AC8 reworded the string). (4) AC3 catch `PaymentGatewayException | TransientDataAccessException` → `PaymentGatewayException | DataAccessException`: `CaptureReservationIT` (`skillars-deferred-58` AC1) forces a `DataIntegrityViolationException` (non-transient) from `deductSession` and requires the `REQUIRES_NEW` `persistPaymentFailure` record to survive — the transient-only catch missed it and propagated raw. Widening keeps genuine bugs (NPE/IllegalState/IllegalArgument) propagating; `CreditRoutingTest`'s transient + programming-bug cases both still hold. (5) **Unrelated environmental gate:** `pom.xml` `<netty.version>` `4.1.136.Final` → `4.1.137.Final` — Trivy's pr-build image scan newly flagged `CVE-2026-75595` (`io.netty:netty-handler`, CRITICAL) on 2026-09-09; `4.1.137.Final` is the named 4.1.x fix. Follows the existing "Security overrides of the Boot parent's managed versions" block. `mvn verify` (full surefire + failsafe) was green on the run that surfaced the Trivy failure. |
