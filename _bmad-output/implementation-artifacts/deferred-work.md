...
`requirements/deployment/local/local-manual-testing.md`, `docs/deployment/local-deployment.md`). One item
closed: the "ses-1-2 code review" bullet noting `notification/index.html:111` still placed `MailSenderProvider`
in the notification module — ses-1.7 AC1 Task 1.3 deleted that row and cross-referenced the infrastructure
doc instead; deleted. All other file references found (envelope_entity Flyway gap, credential-exposure-surface
note, HCLOUD_TOKEN ops note) are either still-open or explicitly pre-existing/dismissed and unaffected by a
...
Full pass over every bullet `skillars-deferred-110`'s ACs cite as their ledger target (see the story file's own
per-AC `Ledger:` lines), cross-checked against the merged code (`master@64c08877`, PR #188). Nine bullets
closed and deleted, one decided-and-annotated (not deleted — no code change), the rest confirmed still open
and left in place:

- **Closed/deleted:** ses-1-1's "`NoHardcodedSenderTest` only inspects the first `from-address` match" (AC1);
  ses-1-2's "`SmtpErrorClassifier` never inspects `getFailedMessages()`" (AC2), round-robin counter overflow,
  empty-provider-list divide-by-zero, and "no `SmtpPropertiesValidator` counterpart" (all three AC3); ses-1-3's
  "`EmailTransportPermanentException` still retried 3x" (AC4, owner-approved widening); ses-1-4's "`envelope_entity`
  has no Flyway DDL" (AC9 — `V136__pin_envelope_entity_schema.sql`); `skillars-deferred-109`'s
  `booking.store.js`/`handleAcceptAllBatch` delete-guard bullet (AC6, downgraded to defensive-invariant but
  implemented) and its CI-timezone bullet (AC7 — matrix landed); `ses-1-7-documentation`'s `app.email.log.outbox-dir`
  naming-collision, unbounded no-boolean-flag claim, and undocumented `email.providerConfigs` bullets (AC10/AC11).
- **Decided, not deleted:** ses-1-4's `EmailTemplate.valueOf` poison-row bullet (AC8) — owner explicitly chose
  to keep retain-and-alert (matches `OutboxRowProcessor`'s "never drop" precedent for the missing-handler case);
  annotated `[DECIDED: keep retain-and-alert — skillars-deferred-110]` in place rather than deleted, since no
  code changed and the decision itself is worth keeping to prevent re-litigation.
- **Rewritten, still open:** `ses-1-7-documentation`'s `envelope_entity` Flyway-callout-severity bullet — the
  doc-only complaint (severity framing + missing cross-reference link) is unaddressed, but its premise
  ("missing migration") is now stale since AC9 landed `V136`; reworded to point at the migration instead of
  describing it as absent.
- **Explicitly held, confirmed still open (no change):** `skillars-deferred-109`'s `VideoModerationEmailListener`
  `persisted == null` bullet (AC5 — root cause still unestablished, diagnostic-only WARN enrichment added) and
  its `MailManager.isRetryable`/circuit-breaker bullet (explicitly scoped out of the bundle as needing its own
  investigation-led story). Also confirmed still open and untouched: ses-1-2's §7.2 item 13 adapter-wrap-depth
  guard; ses-1-3's DOWN-health-caching TTL, mid-loop rate-limit-duplication, and rate-limit WARN-log-level
  bullets; ses-1-4's `Map.of`→`HashMap` null-token guard and AC7 log-masking-bypass and `RegistrationEmailDurabilityIT`
  global-state bullets — none were in scope for `skillars-deferred-110` and none were touched by it.
- **New, correctly still present:** the `implicitTls` bullet (surfaced during `skillars-deferred-110`'s own
...
- **[Narrowed by skillars-deferred-129 AC1, 2026-09-22] No `lock_timeout`/`statement_timeout` bounds
  ...
  **[AUDIT 2026-09-23 (skillars-deferred-130): re-confirmed `[DECIDED: accepted risk —
  skillars-deferred-128]` for a fourth consecutive story.** Investigated as a possible AC and declined
  as a code-change item (owner decision, `AskUserQuestion`, this story's own creation session) after
  discovering the "scoped connection-timeout override" originally imagined as a small fix is not one:
  HikariCP has no per-call-site connection-acquisition-timeout override through the standard
  `DataSource.getConnection()` path Spring's transaction manager uses (`connection-timeout` is
  pool-wide only). The only real fix shape is a second, dedicated `HikariDataSource` scoped to
  `deletePlayerDevelopmentData`'s own `REQUIRES_NEW` transaction — which would also need separate
  wiring for the Testcontainers `@ServiceConnection` test path (`DataSourceConfig`'s custom
...
`skillars-deferred-130` closed out the "`platform.marketplace`/`platform.reviews` never audited"
narrative that stories 126, 127, 128, and 129 each recorded (each explicitly declined a fresh
concurrency/TOCTOU audit of these two modules in favor of ledger items) — this story is that audit.
Checked 0 `@Scheduled` methods, 26 `@Transactional` annotations across 10 service classes, 11
entities (`CoachAgeGroup`, `CoachAvailabilityWindow`, `CoachMediaItem`, `CoachPricing`,
`CoachProfile`, `CoachReliabilityStrike`, `CoachSpecialty`, `CoachSubscription`, `SessionPack`,
`CoachReview`, `ReviewFlag` — corrected from an initial miscount of 8 during the original
creation-session audit pass). Both modules are now confirmed swept under this series' concurrency
lens. Findings, corrected during this story's own senior-dev review before implementation began (see
the story file's own Change Log for the full corrections): **AC1 Fix 1 (High, fixed)** —
`ReviewFlagService.flag`'s auto-hold write took no lock, refresh, or re-check before mutating
`CoachReview.moderationStatus`, unlike every other writer of that field in the module; the reachable
trigger is `ReviewSubmissionService.updateReview`, the only status-writer that never calls
`ReviewFlagRepository.resolveAllOpenFlags` — fixed by taking `findByIdForUpdate` as `flag()`'s own
first read (not a lock-after-insert, which would deadlock two concurrent flaggers via the
`review_flags` → `coach_reviews` non-deferrable FK). **AC1 Fix 2 (withdrawn and replaced, code review
2026-09-23, Decision 1)** — the originally-shipped `coach_subscriptions_pkey`-catch was itself dead
code: `CoachSubscription` has an assigned `@Id` with no `@GeneratedValue`/`@Version`, so
`save()`/`saveAndFlush()` always routes through `em.merge()`, never `em.persist()` — a PK violation
can never actually be thrown here, concurrent or not. Worse, an unconditional insert would silently
*overwrite* an existing `PRO`/`ELITE` subscription row down to `SCOUT`: `SubscriptionService
.subscribeCoach` has no `DRAFT`/`ACTIVE` guard, so a still-`DRAFT` coach can already hold a real paid
marketplace subscription row before ever publishing. Replaced with find-or-create
(`coachSubscriptionRepository.findByCoachId(...).orElseGet(...)`), which preserves an existing row's
tier untouched and defaults only a genuinely new row to `SCOUT`; race-freedom now comes from AC1
Fix 3's own lock, not from catching a violation that could never fire. **AC1 Fix 3
(Medium, fixed, found only during this story's own review — not the original creation-session
audit)** — `publishProfile` took no row lock at all, so a concurrent
`AdminCoachEnforcementService.suspendCoach` could be silently reverted back to `ACTIVE`; fixed via
this module's own `findByIdForUpdate` + refresh + re-check-`DRAFT` pattern — the marketplace module's
2nd `.withBoundedRetry(` call site (`saveStep4` has the first), the 32nd codebase-wide
(`PessimisticLockRetryerCallSiteAuditTest.EXPECTED_CALL_SITE_COUNT` bumped 31 → 32), and the 14th of
`CoachProfileRepository.findByIdForUpdate`'s own wrapped call sites specifically (code review
2026-09-23: corrected from an original "8th" miscount, which itself compounded a pre-existing stale
"7" in that repository's own comment — also now corrected). **AC2 (fixed)** — see the `D2` annotation
directly above:
...
- **D2 — Declared config bounds and enforced config bounds are stated in two places and can
  ...
  **[AUDIT 2026-09-23 (skillars-deferred-130): PARTIALLY CLOSED, corrected framing.** The literal-bounds
  shape above is not a divergence risk — it is this codebase's own documented convention
  (`ConfigBounds.java:33-35`): call sites intentionally re-type bounds as literals so a `Mockito
  verify(...)` in the call site's own unit test pins the exact numbers as a drift detector, exactly
  matching the cited `RADAR_COMPOSITE_LOCK_TIMEOUT_SECONDS` sibling. The genuine gap this bullet named —
  no such pinning test existed for this call site — is now closed by `GdprErasureServiceTest`
  (`skillars-deferred-130` AC2). Two residuals remain open, NOT closed by this story: (a) the code
  default `5` still appears nowhere in `ConfigBounds` despite the key being registered in
  `HAS_CODE_DEFAULT` — `BoundedKey` has no default field, a separate pre-existing gap from the literal
  re-typing this bullet described; (b) `ReviewFlagService.java:76` / `ReviewSubmissionService.java:165`
  re-type both bounds *and* the key string for their own `getBoundedInt` calls, found during this
  story's own review — out of this story's scope (its AC2 targeted `GdprErasureService`'s call site
  only).]**
  **[CLOSED by skillars-deferred-132 AC3 Fix 10, residual (b) only]** — both call sites now reference
  `ConfigBounds.REVIEWS_AUTO_HOLD_FLAG_THRESHOLD.key()`/`ConfigBounds.REVIEWS_SUBMISSION_WINDOW_DAYS.key()`
  instead of the raw string literal; the bounds themselves (`1, 1000` / `1, 365`) stay re-typed as
  literals, per this codebase's own documented drift-detector convention (`ConfigBounds.java:33-35`) —
  not a residual, a deliberate match to the sibling pattern. Pinned by new
  `ReviewFlagServiceTest`/`ReviewSubmissionServiceTest` `Mockito verify(...)` assertions on the exact key
...
- **D5 — The `lock_timeout` bound is re-read from `ConfigService` once per child, inside `erase()`'s
  ...
  **[CLOSED by skillars-deferred-132 AC2 Fix 9, PARENT branch only]** — `lockTimeoutSeconds` is now
  read once at the top of `eraseParentChildren` (before its loop, unconditionally — even for zero
  children) and passed down as a parameter into `deletePlayerDevelopmentData(playerId,
  lockTimeoutSeconds)`, closing the N-children repetition this bullet named. The PLAYER branch's own
  equivalent read is a no-op change in practice — that path already called
  `deletePlayerDevelopmentData` exactly once, so there was no repeated-read hazard to remove there.
  **This mitigates frequency, not the underlying hazard class**: `ConfigService.refreshCache()`'s
  `private synchronized configRepository.findAll()` stalling every other caller in the JVM while this
  thread holds a lock and a pooled connection is unchanged in kind — hoisting the read makes it happen
  once per `erase()` call instead of once per child, reducing probability, not eliminating the risk.

## Deferred from: code review of skillars-deferred-130-marketplace-reviews-concurrency-audit-and-config-bounds-fix (2026-09-23)
...
- **Lock-order inversion between `ReviewFlagService.flag` and `GdprErasureService.erase`** —
  ...
  `updateRatingAggregate`); `erase()` writes `coach_profiles` first (`:172`) then `coach_reviews`
  (`:182-183`). Unreachable today only because the row sets are disjoint by construction (a coach cannot
  ...
  **RE-CONFIRMED still open (skillars-deferred-132 AC1 Fix 1, 3rd consecutive story):** the reachability
  argument is now stronger and no longer depends on a guard that turned out not to exist —
  `erase()`'s own `coach_profiles` lock only runs inside an `ifPresent` branch, so a non-coach `u` takes
  no `coach_profiles` lock at all, and a coach `u` can author zero reviews (`AuthorRole` excludes
  `COACH`; `ReviewResource.resolveRole` resolves `ROLE_COACH` before any other role, so
  `AuthorRole.valueOf("COACH")` always throws before a coach-authored review can exist) — the two lock
  sets are disjoint by construction for both account shapes, not merely "not the same row pair" for an
  incidental reason. Still not fixed: reordering `erase()`'s writes to close this defensively would
  require auditing every other table it touches for the same risk, a larger change than this latent,
...
- **`CoachReviewRepository.findByIdForUpdate` diverges from the documented NOWAIT + `PessimisticLockRetryer`
  ...
  `CoachProfileRepository.findByIdForUpdate` (`:35-38`), whose `jakarta.persistence.lock.timeout = "0"`
  (NO_WAIT) plus bounded retry is documented at `:28-34` as this codebase's deliberate contention
  strategy. No runtime `lock_timeout`/`statement_timeout` is set anywhere in `src/main/resources`, so
  `POST /api/reviews/{reviewId}/flag` now blocks for the full duration of any concurrent admin
  `approveReview`/`blockReview` or author `updateReview` transaction on that review, and flaggers of one
  hot review fully serialize. **[DECIDED 2026-09-23 (owner, `AskUserQuestion`, code review of
  skillars-deferred-130): accepted for now.** `flag()` stays consistent with the reviews module's three
  existing blocking lockers (`ReviewSubmissionService.updateReview`, `AdminReviewService.approveReview`/
  `blockReview`, `ReviewModerationService`). Adding NOWAIT means changing a shared repository method used
  by four call sites, wrapping each in `PessimisticLockRetryer`, and adding a contention test per site —
  module-wide work that does not belong in a narrow lock-fix story. Revisit if flag-endpoint latency or
  admin-moderation contention is observed in production, or when the reviews module is next opened for a
  locking change.**]
...
- **AC1 (6 items):** Fix 1 (lock-order inversion, `ReviewFlagService.flag` ↔ `GdprErasureService.erase`)
  — **documentation-only, RE-CONFIRMED still open** for a 3rd consecutive story, with a stronger
  disjoint-lock-sets argument (contract-enum membership + role-precedence in the API layer) replacing
  the original argument's now-nonexistent guard. Fix 2 (`ReviewFlagService.flag()`'s blocking lock →
  NOWAIT + `PessimisticLockRetryer`, via a new `findByIdForUpdateNoWait` method used only by that call
  site — CLOSED). Fix 3 (marketplace-tier reconciliation sweep, `SubscriptionTierReconciliationScheduler`
  — CLOSED; the corrected `Enum`-vs-`String` comparison a pre-drafting fact-check caught is load-bearing,
  not cosmetic). Fix 4 (GDPR erasure `REQUIRES_NEW` connection-acquisition bound — CLOSED via a same-
  thread `HikariPoolMXBean` pre-check, not the costlier secondary-EMF or ruled-out cross-thread `Future`
  mechanisms; see Fix 4's own code comment for the full tradeoff). Fix 5 (`PessimisticLockRetryer` per-
  call-site retry attribution via a `lockName` parameter, not an overload — CLOSED). Fix 6 (empirical
  CI-deadlock reproduction attempt — CLOSED; not reproduced in 9 valid local runs, mechanism status
  unchanged: closed by structural reasoning, not exhaustively proven).
- **AC2 (3 fixes, all closed):** Fix 7 (`submitReview` **and** `ReviewFlagService.flag()` →
  `REQUIRES_NEW`, widened from the original single-method finding once the identical mirrored trap was
  found in the sibling method). Fix 8 (`PerformanceReport` projection query, dead method deleted). Fix 9
  (single config read per `erase()` call for the PARENT branch, passed down as a parameter — closes the
  N-children repetition only; the PLAYER branch's own read was already single, and the underlying
  `ConfigService.refreshCache()` hazard class is unchanged in kind, only less frequent).
- **AC3 (closed):** Fix 10 (both review config call sites now reference their existing `ConfigBounds`
  constants by key; bounds stay re-typed as literals, per this codebase's own convention).
  `BoundedKey`'s missing `default` field stays explicitly open, per this story's own owner decision
  (narrower/cheaper to fix only the two call sites than widen `BoundedKey` itself).
- **AC4 (2 fixes, both closed):** Fix 11 (`PessimisticLockRetryerCallSiteAuditTest` now also audits lock
  sites, not just retry-wrapper sites — scoped to the confirmed NOWAIT repositories, with four
  genuinely-blocking repositories named as an explicit exemption list; verified to catch a deliberately-
  introduced unwrapped call). Fix 12 (the two previously-untested `(branch × reason)` combinations —
  PARENT × `CHILD_DELETE_LOCK_TIMEOUT`, PLAYER × `CHILD_CONTENDED` — now covered in `GdprErasureIT`).
- **AC5 (this section):** ledger closeout. Three items are explicitly triaged, NOT silently closed
  over: **Fix 1**'s lock-order inversion (re-confirmed still open, see AC1 above); the **`BoundedKey`
  missing-`default`-field residual** (still open, out of this story's narrower scope); the **Stripe →
  payment reconciliation sweep** the ledger separately calls for (still open — distinct from Fix 3's own
  payment → marketplace sweep, and not closed by it; a coach with a live Stripe subscription and no
  local `payment.coach_subscriptions` row at all is untouched by Fix 3). A fourth, narrower item —
  `persistCoachTierUpgrade`'s own missing catch-and-defer around `syncMarketplaceTier` — was found
  during this story's own review (a live billing-divergence path Fix 3's original draft never traced)
  and was fixed as part of Task 4 (the "recommended" branch of that task's own owner decision), mirroring
  `persistCoachSubscription`'s identical Decision 3 pattern — not left open.
