...
- **`PlaybackServiceIT`'s replacement perf ceiling has near-zero regression value.** AC5's `assertThat(p99).isLessThan(5_000L)` passes a change that takes p99 from ~5ms to 4900ms — an accidental N+1, a dropped index, or a synchronous provider call adding 100-500ms all slip through. The `log.info` of p50/p95/p99 has no baseline file, no trend gate and no consumer. Not a defect: AC5 explicitly sanctioned the loose ceiling ("a genuine pathology, not jitter") and AC10 already filed the missing perf-trend job as a residual. A jitter-tolerant shape assertion (`isLessThan(Math.max(200L, p50 * 20))`) would restore detection of a uniform slowdown without reintroducing the wall-clock flake. [`src/test/java/com/softropic/skillars/platform/video/service/PlaybackServiceIT.java:144-146`]
- **`ApiAdvice.integrityViolationHandler` throws NPE on a null constraint name.** `CONSTRAINT_MAPPINGS` is a `Map.of(...)` and `CONFLICT_CONSTRAINTS` a `Set.of(...)`; `ImmutableCollections` throws `NullPointerException` on `get(null)` / `contains(null)`. Hibernate's `PostgreSQLDialect` extractor only templates a constraint name for a subset of SQLSTATEs, so a 23502/23514 from `phone_otp_tokens` yields `null` and the exception handler itself blows up — generic 500 with a stack trace instead of the clean error body. Pre-existing; skillars-deferred-89 AC7 only adds unauthenticated ingress to it via the two new `permitAll()` `/resend-otp` endpoints. Fix is a null guard before the two lookups. [`src/main/java/com/softropic/skillars/platform/security/api/ApiAdvice.java:159-177`]
...
- **AC1 has no integration-level collision test (the IT was backed out on master, 2026-09-01).** `SluCalculationServiceIT.saveSluWithRetry_concurrentDeliveryCollisionOnV47_isCleanNoOp_noRecoverError` was removed after it broke the first post-merge `master` CI run (PR #134 green, master run `33479590672` red): its worker thread could outlive the assertions on a slow runner, and `pool.shutdownNow()` does not wait for it, so the worker's blocked INSERT committed rows for the shared `TEST_PLAYER_ID` *after* `@AfterEach` cleanup — polluting `onBookingCompleted_draftSession_writesNoRows` (`expected: 0 but was: 2`). AC1's catch → `ALREADY_PERSISTED` behaviour stays covered by `SluPersistenceRetrierTest` (unit) and its snapshot-gating effect by `SluPersistenceDispatcherTest`. **Owed:** a hardened, orphan-free concurrency IT — manage the "winner" JDBC connection manually so `finally` can `rollback()` it *before* draining the worker `Future`, always `loser.get(...)` then `awaitTermination` before returning, and hard-delete `WHERE session_id = ?` in the same method — and ideally drive it through `onBookingCompleted` → `dispatchSluPersistence` → `player_slu_weekly_snapshot.total_slu` as AC1's Tests section originally specified. [`src/test/java/com/softropic/skillars/platform/development/service/SluCalculationServiceIT.java`]
...
- **`user=` with an empty value counts as authenticated.** `startsWith('user=')` is true for a zero-length value, exactly as the old `includes('user=')` was — so the 1.7a robustness fix did not close it. The backend writes the cookie from the `displayName` claim with no null guard, and `ResponseCookie.from(name, null)` coerces to `""`, so a principal with a null display name yields `user=` on the wire and starts session monitoring for an effectively unnamed session. Pre-existing; the diff touched this line without guarding it. [`src/frontend/src/App.vue:22`, `JwtManagerImpl.java:215-216`, `CookieUtil.java:17-25`]
...
- **Legacy fallback fails open on the 401 path.** `JwtManagerImpl.deleteLoginToken` (`:150`) removes `SESSION_REFRESH_COUNTDOWN`, and `securityUtil.logout(res)` runs immediately before `writeUnauthorized`. So on a 401 the `rint` cookie is gone by the time the axios response interceptor calls `refreshExpiryState()`, which then takes the missing-cookie branch and computes `LEGACY_SESSION_TTL - (now - lastActivityTime)` ≈ 15 minutes — for a session that is provably dead. The request interceptor's `recordActivity()` on the same request guarantees `lastActivityTime` is fresh, so the fallback re-arms a full window on exactly the responses that prove expiry. Not live today because the `errorKey` gate at `boot/axios.js:146` fires on that same response and tears the session down (verified), so this is defense-in-depth only — but it removes the elapsed-time safety net that previously covered a stripped/blocked response body. Fix would need a "have we ever seen `rint`?" flag to distinguish "never issued" from "just cleared". [`src/frontend/src/plugins/sessionManager.js:computeTimeUntilExpiry`, `JwtManagerImpl.java:150`]
- **No i18n keys for `security.sessionExpired` / `security.unauthorized`.** `JWTAuthorizationFilter.writeUnauthorized` calls `messageSource.getMessage(errorKey, null, defaultMsg, locale)`, but neither key exists in `messages.properties`, `messages_en/de/fr.properties`, or `error-messages.properties` — only `security.msg.unauthorized` and the `security.emailToken*` family do. So the lookup always falls through to the hardcoded English default, and DE/FR users see English on the main session-expiry path. `project-context.md` requires user-facing text to be externalized. Pre-existing `ApiAdvice` pattern (it has the same gap), and the story disclosed it, but the filter now puts it on the most-travelled auth path. Fix is three bundle entries per key. [`src/main/resources/i18n/`, `JWTAuthorizationFilter.java:writeUnauthorized`]
...
- **A sibling tab keeps rendering an authenticated UI after another tab logs out.** Once `rint` is gone, `computeTimeUntilExpiry()` returns the purely local `LEGACY_SESSION_TTL - (now - lastActivityTime)`, and the axios request interceptor resets `lastActivityTime` on every outgoing request — so the estimate re-arms a full 15 minutes indefinitely. `App.vue` evaluates `isAuthenticated()` only at mount, and nothing in `tick()` re-checks that the session cookies still exist. Trigger: two tabs open; Tab A logs out or 401s, and the server's `deleteLoginToken` clears `rint`/`user`/`potc`/`skp` for the whole browser; Tab B never notices and shows a live UI for a dead session until a click 401s into a hard `window.location.href` reload. Same root cause as the round-1 "fallback fails open" entry above, but reached without any 401 arriving in the affected tab. A cookie-presence re-check in `tick()` would close both. [`src/frontend/src/plugins/sessionManager.js:computeTimeUntilExpiry`, `src/frontend/src/App.vue`]
- **`refreshSession()` failure is invisible to the user.** The `catch` only calls `console.error`; nothing sets an error flag and `SessionWarningDialog.vue` has no error surface, while `isRefreshing` returns to `false` so the button re-enables as though the extension succeeded. Trigger: user clicks "Continue session" at 2:00 remaining and the request fails (network blip, or a revoked-refresh-token 401). The spinner runs and stops, the counter keeps ticking down, and at 0:00 the user is logged out having been told nothing. Pre-existing — this catch block predates the review — but the 1.7b dialog makes it much more visible. [`src/frontend/src/plugins/sessionManager.js:refreshSession`, `src/frontend/src/components/common/SessionWarningDialog.vue`]
...
- **Filter 401s emit `helpCode: null` and skip `ApiAdvice`'s logging/alerting side-effects.** Every `ApiAdvice` error path routes through `toErrorDTO`, which is always given a `helpCode` generated by `logError(...)`, and the access-denied path additionally publishes a `SecurityAlertEvent`. `JWTAuthorizationFilter.writeUnauthorized` constructs `new ErrorDto(null, new ErrorMsg(errorKey, message))` directly, so auth failures caught in the filter produce no help code, no error log line and no security alert. Field names and nesting do match, and no frontend consumer reads `helpCode` on a 401, so this is an observability gap rather than a contract break. [`JWTAuthorizationFilter.java:writeUnauthorized`, `ApiAdvice.java:toErrorDTO`]
