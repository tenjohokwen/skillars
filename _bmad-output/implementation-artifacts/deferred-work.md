...
- **`BookingBatchStatusListener` has no failure isolation and no null-check on `event.bookingId()`.** The AFTER_COMMIT listener is not `@Async` and has no try/catch, so a `PessimisticLockingFailureException` (or any `RuntimeException`) from `updateBatchStatusFromBooking`, or a null `bookingId()` → `findById(null)` `IllegalArgumentException`, propagates out of the committed transaction's `commit()`; the batch status is left permanently stale with nothing to detect it. Pre-existing — deferred-100 AC4 only added the `@Transactional(REQUIRES_NEW, readOnly = true)` wrapper. [`BookingBatchStatusListener.java`]
- **`VideoService.retryUpload` orphans the pre-retry provider (Bunny) asset.** The prior `providerAssetId` — whose tracking row was already deleted on the original upload's success — is overwritten with the retry's new id; the old asset is invisible to both `ReconciliationWorkerScheduler.sweepOrphanedProviderAssets` (no tracking row) and `reconcile()` (the `Video` now points at the new id), so it bills and stores PII indefinitely. Explicitly descoped by deferred-100 AC2 (spec + inline comment). [`VideoService.java:180`]
