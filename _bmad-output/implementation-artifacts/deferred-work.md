...
- **`SnapshotPersistenceRetrier` / `SluPersistenceRetrier` only retry and `@Recover` `DataAccessException` — transaction begin/commit failures are neither retried nor recovered.** A failure raised at `@Transactional` boundary (`CannotCreateTransactionException`, `TransactionSystemException` — subclasses of `org.springframework.transaction.TransactionException`, which does **not** extend `DataAccessException`) is not matched by `@Retryable(retryFor = DataAccessException.class)` nor by the `@Recover(DataAccessException ex, ...)` method, so it propagates raw out of `SluCalculationService.onBookingCompleted` (`@Async @TransactionalEventListener`) into `SimpleAsyncUncaughtExceptionHandler` as a bare stack trace — the intended `"Failed to write SLU weekly snapshot after retries — rows lost, manual recovery needed"` log never fires. skillars-deferred-84 AC2 was directed to mirror `SluPersistenceRetrier` exactly, which carries the identical gap, so this is a pre-existing pattern issue affecting both retriers, not a regression. A fix would widen `retryFor` and add a second `@Recover` overload (or broaden the throwable) across both classes. [`src/main/java/com/softropic/skillars/platform/development/service/SnapshotPersistenceRetrier.java:39-53`, `src/main/java/com/softropic/skillars/platform/development/service/SluPersistenceRetrier.java:28-44`]

- **No test exercises the actual retry / `@Recover` proxy path for any SLU retrier.** All three `SnapshotPersistenceRetrierTest` tests (and the mirrored `SluPersistenceRetrierTest` before them) instantiate the bean with `new`, no Spring context — `writeAllWithRetry_writerThrows_propagatesToCaller` explicitly asserts the *un-proxied* (no-retry) behaviour. Nothing anywhere asserts `verify(writer, times(3))`, exponential backoff, `retryFor` matching, or that `@Recover` is reached via the proxy after exhaustion. Deleting `@Retryable` / `@Recover` / `@EnableRetry`, changing `retryFor`, or drifting the `@Recover` signature all pass CI green. A single `@SpringBootTest` + `@EnableRetry`-backed test proving retry-then-`@Recover` on exhaustion would cover both retriers. [`src/test/java/com/softropic/skillars/platform/development/service/SnapshotPersistenceRetrierTest.java`, `src/test/java/com/softropic/skillars/platform/development/service/SluPersistenceRetrierTest.java`]
