...
- **`@Backoff` retry sleeps run on the `@Async` booking-completed listener thread pool with no bulkhead.** `SluCalculationService.onBookingCompleted` is `@Async @TransactionalEventListener` and calls `sluPersistenceRetrier.saveSluWithRetry(...)` then `snapshotPersistenceRetrier.writeAllWithRetry(...)` synchronously. Under a DB hiccup during a burst of booking completions, each listener thread now parks through *two* sequential backoff chains (100 ms → 200 ms each by default) before failing over, so SLU + snapshot processing for all other players stalls behind the sleeping threads. `writeAllWithRetry` is not itself `@Async` and there is no bounded queue / bulkhead. Pre-existing architecture — `SluPersistenceRetrier` was already invoked this way; skillars-deferred-84 AC2 added the second chain. [`src/main/java/com/softropic/skillars/platform/development/service/SluCalculationService.java:49-51,177,185`]
