...
- **The `.env` guard is duplicated verbatim across 4 scripts instead of a shared sourced snippet or function.** A future 5th backup/restore script can "forget" the pattern since there is no single place enforcing it. Matches `prune-backups.sh`'s established per-script convention; the story's own Dev Notes explicitly said not to invent a new style, so not fixed here. [`deploy/backup/pg-backup.sh`, `deploy/backup/volume-backup.sh`, `deploy/backup/restore-from-volume-backup.sh`, `deploy/backup/restore-from-dump.sh`]
