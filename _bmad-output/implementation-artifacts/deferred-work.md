...
- **`envelope_entity` has no Flyway DDL and its `send_id` unique index is unversioned.** No `CREATE TABLE envelope_entity` exists in any of the 129 files under `src/main/resources/db/migration/`, nor in `src/test/resources/sql/createSchema.sql`; `spring.jpa.hibernate.ddl-auto: none` (`application.yaml:66`) with `flyway.baseline-on-migrate: true`. The table and its unique constraint therefore exist only by out-of-band provenance. ses-1.4's AC4 (`MailManager.saveAndFlush`) and the new `MailManagerDuplicateSendIdIT` both now depend on that constraint firing, which raises the stakes on an already-known gap — the project's own `docs/dev-docs/notification/index.html:309` records it ("worth confirming your local schema actually has it"). Fix is an additive expand-phase migration creating the table `IF NOT EXISTS` plus a guarded unique index on `send_id`, per `docs/deployment/migration-conventions.md`.
- **`EmailTemplate.valueOf(p.template())` creates an immortal poison outbox row for a removed or renamed constant** (`platform/notification/service/NotificationEmailOutboxHandler.java:90`). Payloads carry the template as a `String` and can sit in `outbox_messages` for up to the 24h default deadline, so a rename during a rolling deploy makes `valueOf` throw `IllegalArgumentException` — thrown *after* ses-1.4's new deadline guard, so a still-live row is never rescued by it. `OutboxRowProcessor` wraps it, backs the row off and retries forever, consuming a claim slot and eventually reporting `[OUTBOX_STUCK]` with no path to completion. `OutboxRowProcessor:109-113` already handles the analogous case for a *missing handler*; the missing enum constant has no equivalent guard and, unlike a missing handler, can never resolve itself. Pre-existing line, unchanged by ses-1.4.
...
- **`app.email.log.outbox-dir` collides in name with the transactional outbox.** `docs/dev-docs/infrastructure/index.html` documents the log transport's filesystem dump directory as `app.email.log.outbox-dir`, in the same cross-linked doc set that introduces `platform.outbox`'s transactional outbox table. Anyone grepping `outbox-dir` while investigating outbox-row delivery gets a false lead into a transport that is active in neither dev nor prod. The collision is in the property name itself, not the doc — a rename (or a disambiguating clause in both pages) is the real fix. Not introduced by ses-1.7.
- **`envelope_entity` Flyway callout does not reflect its raised severity.** `docs/dev-docs/notification/index.html:336`'s callout still frames the missing `CREATE TABLE envelope_entity` migration as an audit-history gap. Since ses-1.4, registration and OTP mail route through `EnvelopeEntity` for the first time, so a missing table now breaks account verification, not just booking-email history. The callout also cites `deferred-work.md` as a bare `<code>` filename with no path and no `<a href>`, from a page nested at `docs/dev-docs/notification/` — every other cross-reference on that page is a link. ses-1.7's AC1 explicitly scoped this callout to "leave as-is beyond an optional cross-reference," so both points were out of scope for that story. The underlying DDL gap itself remains tracked under `ses-1-4-registration-email-durability`.
- **Unbounded "no boolean enable/disable flag anywhere in this area" claim.** `docs/dev-docs/infrastructure/index.html` asserts this of the email transport area. True of `infrastructure.email`/`.ses`/`.email.smtp`, but `platform/notification/config/ComponentConfig.java:32` still gates a notification bean on `@ConditionalOnProperty(name = "enable.test.mail", havingValue = "false", matchIfMissing = true)`. Different package area, so the claim is defensible as scoped — but the wording is absolute enough that a single surviving toggle falsifies it. Tighten to name the packages it covers.
- **SMTP round-robin provider config (`email.providerConfigs`) is documented nowhere.** ses-1.7 correctly deleted the `MailSenderProvider` row from `docs/dev-docs/notification/index.html` (the class moved to `infrastructure.email.smtp`), and the notification page does cross-reference the Infrastructure dev-doc for the transport package. But the infrastructure page names `MailSenderProvider` without a description, and neither page documents the `email.providerConfigs` key or the gmx/gmail round-robin behaviour it drives. Someone configuring SMTP providers now has a class name with no config key and a config key documented nowhere. Pre-existing gap that the row deletion surfaced; belongs to the infrastructure page's scope.
