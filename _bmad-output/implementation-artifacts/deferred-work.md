...
up in the same pass per the `skillars-deferred-53` precedent, one filed fresh and deliberately left open.
The escrow / completion-gated-payout item was taken up as **skillars-deferred-91 AC5**: Part A (the
`CAPTURE_PENDING` → `CAPTURE_ABANDONED` timeout + slot release) shipped; Part B (completion-gated payout
proper) is a Stripe-Connect model change filed as its own story — see the skillars-deferred-91 residuals
section below and `docs/architecture/payout-and-capture-pending.md`. `BookingService.recordNoShowCoach`
(`:739-758`) stays `UPCOMING`-only per the same 2026-08-24 owner decision.
...
- **[CLOSED by skillars-deferred-91 AC4, 2026-09-03] SLU weekly-snapshot *under*-report direction.** `SnapshotPersistenceRetrier`'s two `@Recover` methods now enqueue the exhausted snapshot write onto the generic outbox (`SluSnapshotOutboxSupport` → `SLU_SNAPSHOT_WRITE` handler → `SnapshotBatchWriter.writeAllDeltas`), so a `@Recover`-swallowed write is re-driven off the request path and is idempotent through the `V119` marker (no over-report). Proven by `SluSnapshotOutboxIT`.
...
- **`AFTER_COMMIT` reliability catalogue — remaining un-migrated listeners.** AC1–AC4 wired the refund (`CancellationRefundService` × 4 `BOOKING_REFUND` calls — AC2), notification-email (`BookingEmailListener` × 18 + `SessionPackEmailListener` × 3 — AC3, wider scope per owner) and SLU-snapshot (`SnapshotPersistenceRetrier.@Recover` — AC4) paths onto the generic outbox. Deliberately **not** migrated: `PendingBlobDeletionService` (its own just-shipped, just-reviewed outbox — a "re-express on the generic outbox" refactor is a nice-to-have, not required); `CancellationRefundService`'s `packSessionService.restoreSession(...)` calls (a pack-session-restore idempotency concern distinct from the money refund); registration-email listeners (`{Parent,Coach,Player}RegistrationEmailListener` — publish inside a live transaction, work today); SSE / video / development / admin / reviews / session `@TransactionalEventListener` sites that are not money- or compliance-relevant.
...
## Deferred from: code review of skillars-deferred-91 (2026-09-03)

- **`AppEndpointsConventionTest` may validate a different request matcher than production.** The test evaluates the `permitAll` patterns with `PathPatternRequestMatcher`, while `SecurityConfiguration.java:231` passes raw strings to `requestMatchers(String...)`. Under `PathPattern` semantics the pre-AC15 patterns (`resend-otp**`) are not valid at all, which implies production was matching with Ant semantics — so AC15's anchoring fix is correct, but the convention test's guarantee does not necessarily transfer to the matcher that actually secures the app. Confirm which matcher `requestMatchers(String...)` resolves to on this Spring Security version and align the test. Deferred: test-fidelity concern, does not affect the shipped fix.
- **The transactional-email `data` map is now type-lossy across the outbox.** `NotificationOutboxSupport.enqueueEmail` serialises `Map<String,Object>` to JSON and `NotificationEmailOutboxHandler` deserialises it back, so any non-string value returns as `String`/`Integer`/`Double`/`List`/`Map`. Verified a non-issue today: all 69 `data.put(...)` call sites across `BookingEmailListener` and `SessionPackEmailListener` put a `String` or a `List<String>`. Deferred: latent — the first `Instant`/`BigDecimal` value added to a template will silently degrade (ISO blob, or `40.0` for `40.00`) with no test to catch it. Consider a typed payload record or a serialisation round-trip assertion when a typed value is first added.

- **The transactional-email outbox enqueue still runs after the producing commit.** skillars-deferred-91 AC3 routed all 19 `BookingEmailListener` + 3 `SessionPackEmailListener` publish sites through the durable outbox, which closed the nested-`AFTER_COMMIT` silent drop (`MailManager`'s own `@TransactionalEventListener(AFTER_COMMIT, fallbackExecution=false)` had no transaction to hang off, so the email was never sent at all). But those listeners are themselves `AFTER_COMMIT`, so `NotificationOutboxSupport.enqueueEmail`'s `REQUIRES_NEW` transaction starts *after* the business transaction has already committed — AC1 requires the producer to enqueue *inside* it. A crash or DB failure in that window still loses the email, and the failure is swallowed by the listeners' `catch (Exception e) { log.error(...) }`. Closing it means moving the enqueue into each of the 22 producing transactions (booking, session-pack and notification paths) — a large refactor, deliberately not attempted in the code-review pass. Contrast AC2's `RefundOutboxSupport`, which *is* called inside the listener's own `REQUIRES_NEW` and is correct. [`NotificationOutboxSupport.java:45`]
