...
- **SLU non-gating perf-trend signal** — `[CLOSED by skillars-deferred-98 AC1b, 2026-09-07]` Per-skill improving/flat/declining trend independent of the neglected-skill gate: `SluDashboardService.getSkillTrends(...)` (read-time OLS over the existing `findByPlayerIdFromWeek` weekly series, no new schema), `GET /api/development/players/{playerId}/slu/skill-trends`, pure `SluTrendClassifier`, frontend `getSkillTrends(...)` client. Unit + integration tests (`SluTrendClassifierTest`, `SluDashboardServiceTest`, `SluSkillTrendIT`). Coach-dashboard trend card (UI polish) intentionally left to `frontend-test-framework-initiative`-era follow-up.
...
- **Smoke step erroring (not `result=fail`) produces a red deploy run with no notification and no auto-revert.** `.github/workflows/deploy.yml` — if the `Smoke test` step itself dies (ssh/sleep failure) rather than completing with `result=fail`, `steps.smoke.outputs.result` is never written, so the `pass`/`fail` branches, `Auto-Revert` (:109), and the `Fail workflow` marker (:189) all skip; the pre-smoke early-failure notifications only fire on `steps.smoke.outcome == 'skipped'` (:196/:207), not `'failure'`. Pre-existing; not introduced by this story. Fix = restructure smoke-failure handling (e.g. guard on `steps.smoke.outcome != 'success'`).
...
- **`SessionPackPaymentService.purchasePack` compensating refund is best-effort only.** `[PICKED UP by skillars-deferred-94 AC17: javadoc clarified to match actual behavior]` `src/main/java/com/softropic/skillars/platform/payment/service/SessionPackPaymentService.java:92-100` — when `createPurchase` succeeds then throws, the `catch` only matches `PaymentGatewayException`; any other exception out of `paymentGateway.refund(...)` (unchecked, or an unwrapped `StripeException`) propagates and skips the `payment.lifecycleFailure` wrap. Even when caught, the refund failure is only `log.error`d — no persisted record for reconciliation. Result in the worst case: player charged, no purchase row, no refund. Pre-existing code behaviour; the javadoc now matches the actual behavior.
...
- **`SmtpHealthIndicator` probes providers serially (~10s × N) with no overall time bound and no caching/TTL.** `src/main/java/com/softropic/skillars/platform/notification/health/SmtpHealthIndicator.java:204-207,247-252` — each provider costs up to `CONNECT_TIMEOUT_MS + SOCKET_TIMEOUT_MS` = 10s and they run one after another, so `/manage/health*` blocks ~10s × provider count and every scrape opens fresh outbound SMTP sockets. Spec Known Limitations #3 already books background caching / TTL as future work; parallelising the probes and bounding the aggregate is the companion improvement. Deferred at code review.
- **SMTPS implicit-TLS providers (port 465) would always report `DOWN`.** `SmtpHealthIndicator.java:247-272` — the check reads a plaintext `220` banner; a 465 server expects a TLS handshake first and sends no plaintext greeting. No 465 provider is configured today (gmx/gmail on 587); revisit if one is added. Deferred at code review.
