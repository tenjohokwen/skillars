...
- **`runbook.md`'s "clear Redis data" emergency procedure targets `appendonly.aof`/`dump.rdb`, which don't match Redis 7's default Multi-Part AOF layout** (`appendonlydir/` containing `*.base.rdb`/`*.incr.aof`/`*.manifest` files instead). Pre-existing — the same wrong filenames were already referenced before this story, just accessed via a `docker run -v ... alpine rm -f` indirection rather than a direct path — so not introduced by `skillars-uat-2`, but worth knowing before anyone actually runs the procedure: as written it would silently no-op on Redis 7's real AOF data.
