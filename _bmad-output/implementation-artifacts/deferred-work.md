...
- Def3: Long arithmetic overflow in `storageUsedBytes + requestedBytes` — theoretical at practical quota sizes (max ~9.2 EB); no guard exists. `[CLOSED by skillars-deferred-99 AC11: QuotaService.exceedsQuota(used, reserved, requested, limit) uses Math.addExact — an overflow logs ERROR and returns a QUOTA_EXCEEDED-class rejection instead of wrapping negative and passing. Applied to check + reserve.]` [`QuotaService.java:check`, `QuotaService.java:reserve`]
...
- Def8: `BandwidthResetService` period drift when job runs late — `bandwidth_period_start` set to `NOW()` on actual run date, not 1st of month; next period boundary shifts accordingly; acceptable drift for non-billing context. `[CLOSED by skillars-deferred-99 AC12: BandwidthResetChunkProcessor stamps bandwidth_period_start = YearMonth.from(ClockProvider now, UTC).atDay(1).atStartOfDay(UTC); both the SET value and the predicate's "current month" are bound params from one captured instant (deterministic under a pinned test clock). New IT.]` [`BandwidthResetService.java:resetMonthlyBandwidth`]
...
- **D5 — `ConfigService`'s 5-minute cache TTL makes a platform-default change look like it did nothing.** `booking.session.defaultDurationMinutes` is read through `ConfigService.getBoundedLong`, and `ConfigService` refreshes on a `@Scheduled(fixedDelayString = "${app.config.cache-ttl-seconds:300}")`. An admin who changes the value and immediately reloads the availability calendar sees the old slot length and will reasonably conclude the setting is broken. `updateConfig` does call `invalidate()`, so a change made *through the config API* is immediate; a change made directly in the database is not. Documented in the V93 migration comment; a real fix is either an admin-UI note or narrowing the TTL, both of which are wider than this story. `[AUDIT 2026-09-04: this bullet carries no [CLOSED]/[STALE]/[WITHDRAWN] tag and was deleted anyway by the 2026-09-04 prune, in violation of that pass's own stated rule (tagged bullets only). Restored. Still open and unchanged — `skillars-deferred-92` AC24 documented the same tradeoff in `ConfigResource.updateValue`'s javadoc ("project-owner decision 6 was to document this, not change the TTL") rather than closing it, so this remains the ledger's only record of the underlying gap.]`
...
- **Session duration / availability windows are resolved once and not re-validated before persistence, on the write-side TOCTOU window that survives every closure filed against this item so far.** `BookingBatchService.createBatch` (and `BookingService.createBookingRequest`) each resolve `requiredDuration` and fetch `windows` once, then compare an `availabilitySignature` against a fresh re-read immediately before the actual persist (`skillars-deferred-71` AC2 single-booking, `skillars-deferred-72` batch) — this narrows the window from "the whole request" to "between the re-check and the commit," it does not eliminate it. `CoachAvailabilityWindow`/`CoachAvailabilityBlock` carry no `@Version` and no locked-read method exists for either anywhere in the codebase, so a coach edit landing in that narrower window remains unseen; full elimination needs lock support on `CoachAvailabilityWindow`, still out of scope. `RescheduleService` remains genuinely unwired to the signature check, by design, per `skillars-deferred-71` AC2's own scope note (no GET-then-POST seam exists for reschedule) — not a gap. `[AUDIT 2026-09-04: restored — the 2026-09-04 prune deleted this bullet outright on the presence of a `[CLOSED (partially) by ...]` tag without reading the tag's own qualifier, which said in terms that the closure "does NOT eliminate it." `BookingBatchService.java:140-141`'s own comment still reads "no @Version/lock support exists on CoachAvailabilityWindow — that remains its own open ledger item," and `CoachAvailabilityWindow` still carries zero `@Version` fields — reverified against source, not carried over from the stale text.]` [`BookingBatchService.java:140-141`, `BookingService.java`, `CoachAvailabilityWindow.java`]
...
- **`BookingService.recordNoShowCoach` only accepts a no-show report while the booking is still in a status the `NO_SHOW_COACH` transition allows from (`UPCOMING`), by design.** `[DECIDED 2026-08-24, restored 2026-09-05: the 2026-09-04 prune deleted the `## Deferred from: skillars-deferred-63 story creation` section wholesale (an untagged narrative block, not a `[CLOSED]`/`[STALE]`/`[WITHDRAWN]` bullet) as one of six sections whose bullets were prune-emptied, taking this record with it. Re-verified against current source: `recordNoShowCoach` (`BookingService.java:837-869`) still routes through the same `transition(...)` state machine and carries no separate status check of its own — the restriction is exactly as this decision described it. The section's other fact — that AC5 Part B (completion-gated payout) is filed as its own story — survives independently in the `skillars-deferred-91` residuals section below and was not actually lost.]` [`src/main/java/com/softropic/skillars/platform/booking/service/BookingService.java:837`]
...
- **Video Error Handling UX** — `[CLOSED by skillars-deferred-99 AC14 (project-owner decision: toast + one auto-retry): useDrillVideoPlayback composable — retry once with a fresh signed URL, then a dismissible toast + inline "unavailable" tile; per-drill state reset; module-level toast de-dup; onBeforeUnmount active-flag guard. session.drillLibrary.videoUnavailable added to en/de/fr.]` When a video load fails (e.g., expired signed URL), code silently refetches drill data in background. Video element remains broken with no user notification. [`DrillCard.vue`, `DrillDetailPanel.vue`, and page handlers]
...
- **First-ever `:latest` publish where GHCR returns `denied` instead of `not found` / `MANIFEST_UNKNOWN` still misses the classification, and the exclusion is deliberate rather than a gap.** `ci.yml`'s manifest-inspect step matches `not ?found|manifest ?unknown|MANIFEST_UNKNOWN|name ?unknown` and falls to the generic "read failed, publish `sha-` only" branch on anything else, so a brand-new package still needs one extra run before `:latest` appears — self-heals, one-run delay. `denied` is deliberately **not** added to that pattern: GHCR also returns `denied` for a permission failure on an *existing* package, and reclassifying it as "package doesn't exist yet" would let a transient permission error trigger an unconditional `:latest` overwrite — the older-overwrites-newer regression the whole guard exists to prevent. `[AUDIT 2026-09-04: restored — the 2026-09-04 prune deleted this bullet outright on a `[CLOSED by skillars-deferred-88 AC6 — for the "name unknown" phrasing only; denied is deliberately NOT …]` tag, reading only the `[CLOSED by ...]` prefix and not the qualifier that immediately follows it. Reproduced against current `ci.yml:195-208`: the `denied` exclusion and its rationale comment are both still live and unchanged. Not a new defect — the still-open half of a partially-closed item, same as the two filed above.]` [`.github/workflows/ci.yml:195-208`]
...
- **No non-gating perf-trend signal for `authorizePlayback`.** `[CLOSED by skillars-deferred-99 AC15: the production `@Observed(name="video.playback.authorize")` + `VideoMetrics.recordPlaybackAuthorizeLatency` on the 4-arg `authorizePlayback` (VideoPlayResource's path) already land in the Prometheus/OTLP pipeline; removed the dead p50/p95/p99 `log.info` loop from `PlaybackServiceIT` and added an observation-registration assertion.]` skillars-deferred-89 AC5 removed the flaky `p99 < 200ms` merge gate and left a `log.info` of p50/p95/p99 in Failsafe output that nothing scrapes. The deferred-23 bullet's own suggested remedy — a dedicated non-gating perf-tracking job that records the distribution over time — is not built. [`src/test/java/com/softropic/skillars/platform/video/service/PlaybackServiceIT.java`]
...
- **`sessionManager.js` state machine rewritten with zero frontend tests.** The diff replaces the whole expiry/warning/countdown state machine (`computeTimeUntilExpiry`, the legacy-fallback branch, `tick()`'s warning edges and expiry dispatch, the `refreshSession()` interaction) and adds backend tests only. Three of the confirmed patch findings in this review — the stale-`rint` parse, the countdown-timer leak, and the unevaluated-state window — are each a one-line unit test with a faked `document.cookie` and `Date.now()`. The repo has no frontend unit-test infrastructure, which the story disclosed, so this is a standing gap rather than a story miss. `[NOTE: the frontend-unit-test gap is consolidated in the frontend-test-framework-initiative backlog item; the early-return tradeoff below is now documented in code with the deferred-90 rationale (verified at HEAD, skillars-deferred-99 AC17).]` [`src/frontend/src/plugins/sessionManager.js`]
...
- **`validation.phone.*` and `auth.nationalId` still describe Cameroon.** `[CLOSED by skillars-deferred-99 AC16
...
- **`MvcConfig.messageSource` `setFallbackToSystemLocale(false)` with no `setUseCodeAsDefaultMessage`** — `[CLOSED by skillars-deferred-99 AC3: setUseCodeAsDefaultMessage(true) + WarnOnMissingMessageSource (bounded warn-once) so a missing dynamic code degrades to code+WARN, not a 500; MessageCodeBaseBundleCompletenessTest gates the literal codes.]` any key missing from every bundle (`messages_<locale>` *and* base `messages.properties`) now throws `NoSuchMessageException` at runtime instead of falling through to the JVM-default bundle. Deliberate and documented (AC12.4), but only safe if `messages.properties` reached genuine full parity in the i18n work. Verify against Chunk 3 (i18n) review; if parity is incomplete, this is a live 500 risk for non-de/fr/en clients. `src/main/java/com/softropic/skillars/platform/security/config/MvcConfig.java:52`
...
- **`MigrationLint`'s reference scan re-walks `src/main` once per dropped identifier, and the real-tree test inherits the fragility the fixture corpus was built to avoid.** `[CLOSED by skillars-deferred-99 AC7: memoised sourceCorpus() — one walk+read per root set; word-boundary matching already landed; referenceScan_underscoreAdjacentName_isNotAMatch regression test added.]` No caching: a migration dropping five columns reads every `.java`/`.sql`/`.yaml`/`.yml`/`.xml` file under `src/main` five times, against the class javadoc's claim that the lint "fails the build in milliseconds". Separately, `MigrationConventionLintTest.realMigrations_aboveBaseline_areClean` scans the live tree, so adding an unrelated class containing a dropped identifier's substring can break the build on a migration nobody touched — the test author identified that fragility and solved it for the fixture corpus (`FIXTURE_SOURCES`) while leaving it in the real path. Largely subsumed by the word-boundary fix filed as a patch in the same chunk; revisit only if that fix does not remove the noise. `src/test/java/com/softropic/skillars/db/MigrationLint.java:508`
...
- **`auth.phoneHintFormat` presents a market-specific MSISDN rule to every locale.** `[CLOSED by skillars-deferred-99 AC16: phone validation is now config-driven per market (app.validation.phone + MarketResolver + PhoneRuleRegistry), default permissive international; auth.phoneHintFormat + auth.nationalId reworded locale-neutral in en/de/fr; backend validation.phone.* neutralised + hintDefault added. Residual: operator-level Provider detection stays CM-only.]` "9 digits starting with 6 (e.g., 670123456)" is translated verbatim into de-DE and fr-FR. Either the rule genuinely applies to all markets (in which case the hint is fine and this is closed), or the phone validation itself is market-specific and both hint and validation need per-market handling. A product decision, not a translation defect. `src/frontend/src/i18n/de-DE/index.js:37`
...
- **The forced-termination escalation covers one thread pool out of seven, and its own rationale applies to all seven.** `[CLOSED by skillars-deferred-99 AC2: GracefulShutdownTaskExecutor now applied to all six ThreadPoolTaskExecutor pools; ExecutorShutdownConfigurationTest fails on a plain one.]` `ExecutorShutdown.gracefulFixedPool().shutdown()` now escalates to `shutdownNow()` when a pool blows its await budget, so non-daemon workers cannot run on against a torn-down application context after a failed `@SpringBootTest`, an `/actuator/restart` or an embedded-container shutdown. Only `storageUploadExecutor` uses that factory. The other six pools — `taskExecutor`, `sendMailPool`, `outboxDrainPool`, `sluRetryExecutor`, `moderationTaskExecutor`, `reportExecutor` — are `ThreadPoolTaskExecutor`s whose timeout is handled by Spring's `ExecutorConfigurationSupport.awaitTerminationIfNecessary`, which (disassembled from `spring-context-6.2.19.jar`) logs at WARN and returns; its only `shutdownNow()` sits on the branch that `configureGracefulShutdown` deliberately turns off. Not patched here because closing it means subclassing `ThreadPoolTaskExecutor` or adding a `BeanPostProcessor` across five config classes, and the worst case would spend 6 s of the 7 s of headroom the 55 s `stop_grace_period` currently has. `src/main/java/com/softropic/skillars/infrastructure/threadpool/ExecutorShutdown.java:215`
...
- **Smoke step erroring (not `result=fail`) produces a red deploy run with no notification and no auto-revert.** `[CLOSED by skillars-deferred-99 AC6.]` The substance shipped in `deferred-96` (`542eab1`): the revert / Slack / email / fail-marker steps now guard on `steps.smoke.outcome == 'failure' || 'cancelled'`, the smoke step writes a `result=error` fallback, and it carries `timeout-minutes` + ssh `ConnectTimeout=10 -o BatchMode=yes`. AC6 closed the one residual: the smoke step now `exit 1`s if it still cannot persist `result` (an unwritable `$GITHUB_OUTPUT` on a *passing* poll would otherwise be a silent green run), and the `Fail workflow` marker also fires on `outcome == 'success' && result != 'pass'`. (`deferred-97` AC3/AC4 own the separate poll-window and ssh-swallow bullets — untouched.)
...
- **`SessionPackPaymentService.purchasePack` compensating refund is best-effort only.** `[CLOSED by skillars-deferred-99 AC1]` — inner catch widened to `Exception`; deterministic Stripe idempotency key on the refund; a durable `payment.stripe_refund_failures` row (`V130`, entity/repo, `RefundReconciliationService` in `REQUIRES_NEW`) written when the refund fails, and a reconciliation-write failure logged as "MANUAL RECOVERY REQUIRED" ERROR while still throwing a clean `payment.lifecycleFailure`. `src/main/java/com/softropic/skillars/platform/payment/service/SessionPackPaymentService.java` — when `createPurchase` succeeds then throws, the `catch` only matched `PaymentGatewayException`; any other exception out of `paymentGateway.refund(...)` propagated and skipped the `payment.lifecycleFailure` wrap, and even when caught the refund failure was only `log.error`d with no persisted record.
...
- **Actuator health endpoint should surface notification-channel reachability.** Follow-up from the deferred-94 AC12 decision (accept `continue-on-error: true` on the deploy-workflow notify steps). Because a failed Slack/email notification no longer fails the deploy run, a down channel is currently invisible until the next incident. Add a custom Spring Boot `HealthIndicator` (or two) that checks SMTP connectivity and the Slack webhook so `/actuator/health` reports `email` / `slack` as `DOWN` when either is unreachable, and wire it into the existing monitoring/alerting.

## Deferred from: code review of skillars-deferred-96 (2026-09-07)

- **`SmtpHealthIndicator` probes providers serially (~10s × N) with no overall time bound and no caching/TTL.** `[CLOSED by skillars-deferred-99 AC5: parallel per-provider probes on a bounded daemon pool under one overall deadline (max(connect+read)+1s, or app.notification.smtp-health.overall-timeout); aggregate Health served from an AtomicReference for app.notification.smtp-health.ttl (default 60s).]` `SmtpHealthIndicator.java` — each provider cost up to `CONNECT_TIMEOUT_MS + SOCKET_TIMEOUT_MS` = 10s and they ran one after another, so `/manage/health*` blocked ~10s × provider count and every scrape opened fresh outbound SMTP sockets.
- **SMTPS implicit-TLS providers (port 465) would always report `DOWN`.** `[CLOSED by skillars-deferred-99 AC5: probeImplicitTlsConnection (SSLSocket handshake) for port 465 / implicit-tls: true.]` `SmtpHealthIndicator.java` — the check reads a plaintext `220` banner; a 465 server expects a TLS handshake first and sends no plaintext greeting. No 465 provider is configured today (gmx/gmail on 587); revisit if one is added. Deferred at code review.
...
**Status:** backlog (its own story — schedule via sprint planning)
**Cited by:** skillars-deferred-98 AC3 (2026-09-07 code review)

Stand up a frontend unit-test runner (Vitest + `@vue/test-utils`): dependency install,
`vitest.config` wired to the Quasar/Vite resolve aliases, a `test:unit` script, CI job, and a
first reference spec. This is deliberately a standalone setup task, not feature-story scope —
it unblocks a batch of recorded coverage gaps at once:

- `skillars-5-4` W9 — `SkillsRadarChartSpec.js` cannot run (ledger line ~722)
- `deferred-17` / `-18` / `-30` / `-37` / `-38` / `-43` D6 — `.vue` / store changes verified by build + code-read only
- `deferred-91` — no coverage for `handleLogout` / i18n `.vue` changes (ledger line ~1293)
- `deferred-90` / `-98` — `sessionManager.js` expiry/warning state machine has zero unit tests
  despite the stale-`rint` parse, countdown-timer leak, and unevaluated-state findings each being
  a one-line unit test with a faked `document.cookie` + `Date.now()` (ledger line ~1261)
- `deferred-35` / `-36` / `-37` / `-38` — `booking.store.js` concurrency guards
- `deferred-12` payment — `PaymentMethodCard.vue` / `payment.store.js` (ledger line ~660)

Once the runner exists, each dependent story reopens its own coverage line as in-scope.
