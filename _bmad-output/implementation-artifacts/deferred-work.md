...
- **D12 — `cancelBookingAsParent`'s locked read races a settle-side plain `UPDATE` that carries no lock-timeout hint.** The `jakarta.persistence.lock.timeout` hint applies only to the locked `SELECT`; a plain `UPDATE` issued by a settle in flight ignores it, so under contention the cancel can block rather than surfacing `ApiAdvice`'s clean 409. Architectural, spanning both the cancel and settle paths, and not a regression introduced by this diff — the pre-story code took no lock at all. [`BookingService.java:602-618`]
...
- **`ConfigService.getBoolean` fails open (returns `false`) for a misconfigured-but-present value guarding security-sensitive gates** (e.g. `AuthService.java:99`'s `security.registration.phone-otp-required`) — now logged at WARN but not alertable. Pre-existing behavior, unchanged by this story's diff (only the log line is new). [`src/main/java/com/softropic/skillars/platform/config/service/ConfigService.java:106-125`]
...
- **No test proves `CoachMediaItem`'s new `@PrePersist onCreate()` callback actually sets `uploadedAt` when the entity is persisted.** Currently unreachable — `grep -rn "new CoachMediaItem()" src/main/java src/test/java` returns zero hits, so nothing constructs/persists this entity yet; add coverage once the coach-media gallery upload feature that will construct it lands. [`src/main/java/com/softropic/skillars/platform/marketplace/repo/CoachMediaItem.java:34-38`]
