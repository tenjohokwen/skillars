...
- No duplicate-booking guard for same slot — multiple REQUESTED bookings for same player/coach/timeslot are possible; credit soft-reservation handles the economic constraint; add a unique partial index on (player_id, coach_id, requested_start_time) WHERE status IN (...) in a future scheduling-conflicts story [BookingService.java:createBookingRequest, V31 migration] `[CLOSED — decided, not open, found during skillars-deferred-65 story creation (2026-08-25): V87__booking_overlap_index.sql's excl_bkg_coach_slot_overlap constraint deliberately excludes REQUESTED as expected, in-band behavior (Story 3.11 follow-up decision, "two overlapping REQUESTED bookings competing for the same slot is expected... not a data-integrity violation") — this item's concern was already decided at the DB-constraint layer, just never annotated here]`
