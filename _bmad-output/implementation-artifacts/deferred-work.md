...
- **AC1's concurrent-delivery IT does not exercise the real event chain.** `SluCalculationServiceIT.saveSluWithRetry_concurrentDeliveryCollisionOnV47_isCleanNoOp_noRecoverError` submits `sluPersistenceRetrier.saveSluWithRetry(losingDelivery)` directly rather than driving two `onBookingCompleted` / `dispatchSluPersistence` calls as AC1's Tests section specifies, and omits the required `player_slu_weekly_snapshot.total_slu` assertion. Net effect: no test covers the collision through `BookingCompletedEvent` → dispatcher → snapshot. Disclosed in Task 1 as a deliberate narrowing (the snapshot assertion belongs to the AC2 path), so a declared gap rather than a hidden one. [`src/test/java/com/softropic/skillars/platform/development/service/SluCalculationServiceIT.java`]
