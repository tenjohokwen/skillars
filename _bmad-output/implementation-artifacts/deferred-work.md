...
- **[PICKED UP by skillars-deferred-93 AC1]** **`components.scss` still has two `transition: all` declarations.** AC25 named only `glass.scss`'s four
...
- **[PICKED UP by skillars-deferred-93 AC2]** **`i18n/error-messages.properties` is very nearly vestigial** (AC12.3, recorded so the audit is visibly
...
- **[PICKED UP by skillars-deferred-93 AC3]** **`marketplace.sortRatingStub` renders "Rating (Epic 9)" to end users**, in all three locales. A
...
- **`MvcConfig.messageSource` `setFallbackToSystemLocale(false)` with no `setUseCodeAsDefaultMessage`** — any key missing from every bundle (`messages_<locale>` *and* base `messages.properties`) now throws `NoSuchMessageException` at runtime instead of falling through to the JVM-default bundle. Deliberate and documented (AC12.4), but only safe if `messages.properties` reached genuine full parity in the i18n work. Verify against Chunk 3 (i18n) review; if parity is incomplete, this is a live 500 risk for non-de/fr/en clients. `src/main/java/com/softropic/skillars/platform/security/config/MvcConfig.java:52`
- **[PICKED UP by skillars-deferred-93 AC4]** **`ModerationSlaMonitorService.detectSlaViolations()` aborts the whole cycle on any non-`TerminalStateViolationException`** — the retry branch has no try/catch around `requiresNewTemplate.execute(...)`; the FAILED branch catches only `TerminalStateViolationException`. An `IllegalStateException` from `enqueueRetry`/`enqueueAdminAlert` (on `JsonProcessingException`) or a `DataAccessException` propagates out of the `for` loop and ends the scheduled run, starving every stuck video after the offender that cycle. Self-healing next cycle unless the fault is deterministic. Pre-existing "one bad video aborts the run" shape — `SessionPackForfeitureScheduler` / `SessionPackExpiryNotifier` catch generic `Exception` per item and this should too. `src/main/java/com/softropic/skillars/platform/video/service/ModerationSlaMonitorService.java:60`
- **[PICKED UP by skillars-deferred-93 AC5]** **Chunked bandwidth reset (`BandwidthResetChunkProcessor`) releases the table-wide lock between 500-row chunks** — the old single `UPDATE` held row locks on every `main.video_quotas` row until it finished, so `QuotaService.reserve()` could not interleave with the monthly reset. It now can: a `reserve()` that increments `bandwidth_used_bytes` on a not-yet-reset row carrying a prior-month `bandwidth_period_start` races a later chunk that zeroes that row. Accounting outcome depends on whether `reserve()` performs its own month-boundary rollover. ~1s window once a month at 00:00 UTC on the 1st. Verify `QuotaService.reserve()` self-heals the period; if it does not, byte accounting can drift for users active in that window. `src/main/java/com/softropic/skillars/platform/video/service/BandwidthResetChunkProcessor.java:1`
...
- **`auth.phoneHintFormat` presents a market-specific MSISDN rule to every locale.** "9 digits starting with 6 (e.g., 670123456)" is translated verbatim into de-DE and fr-FR. Either the rule genuinely applies to all markets (in which case the hint is fine and this is closed), or the phone validation itself is market-specific and both hint and validation need per-market handling. A product decision, not a translation defect. `src/frontend/src/i18n/de-DE/index.js:37`
- **[PICKED UP by skillars-deferred-93 AC7]** **The `MvcConfig.messageSource` finding deferred from chunk 1 is re-scoped, not closed.** Its stated precondition was met: `messages.properties` reached genuine full parity with `messages_en` (130/130, verified against the files with `comm`, not against the completion notes). But the risk it named survives one layer up. Parity is defined against `messages_en`, while the code resolves whatever `EmailTemplate.subjectKey()` returns — and 17 of those 39 keys exist in no bundle, nine of them behind a live Thymeleaf template, where `MailService:74`'s throwing three-arg `getMessage` turns them into `NoSuchMessageException` in every locale. `setFallbackToSystemLocale(false)` does not cause that (a key absent from all bundles threw before the change too) and the HTTP error paths are safe by construction (four-arg defaulting `getMessage`), so this is not a rollback candidate — the outstanding work is the missing subject keys plus a test asserting every `EmailTemplate.subjectKey()` resolves. Both are filed as chunk-3 patches. `src/main/java/com/softropic/skillars/platform/security/config/MvcConfig.java:63`
...
- **[PICKED UP by skillars-deferred-93 AC8]** **The OTP-resend endpoints are `permitAll` and take `userId` from the query string, so the new UI makes account-walking reachable.** `AC28` added a resend control to `/coach/verify-phone?userId=<n>` and `/player/verify-phone?userId=<n>`; both post that id straight to `POST /api/security/{coach,player}/resend-otp`, which is `permitAll`. Anyone can walk `n` and trigger OTP dispatch to arbitrary accounts. Bounded by two buckets — per-IP `@RateLimited(capacity=3, duration=30 MINUTES)` on the resource, and a per-user 3-per-30-min guard in `RegistrationOtpResendSupport:67` — so this is a nuisance and enumeration surface, not an open relay, and the vague `security.otpMismatch` response is deliberately not a rate-state oracle. The `permitAll` design predates this story (`skillars-deferred-89` AC7 shipped the endpoint and nothing called it); AC28 is what made it reachable from the UI. Needs a security decision rather than a dev fix — sign the `userId` into the verification link, bind the resend to the pending-verification session, or accept the risk explicitly and record why. `src/frontend/src/pages/auth/CoachPhoneVerifyPage.vue:106`
