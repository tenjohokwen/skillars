...
## Deferred from: code review of skillars-deferred-85-slu-retrier-transaction-failure-recovery-and-backup-restore-alerting-and-ops-doc-hardening (2026-08-31)

- **SLU snapshot write path has no idempotency guard — a retried ambiguous-commit `TransactionSystemException` double-counts additive upserts.** `skillars-deferred-85` AC1 widened `SnapshotPersistenceRetrier` (and `SluPersistenceRetrier`) `retryFor` to include `TransactionSystemException` / `CannotCreateTransactionException`. `CannotCreateTransactionException` (begin failed → nothing ran) is unambiguously safe to retry. `TransactionSystemException` is not: it is raised on a commit/rollback-phase system error, which includes the case where the DB *did* commit server-side but the client lost the connection/ack. `SnapshotBatchWriter.writeAll` is `@Transactional` and issues only additive `upsertAdd` calls (`total_slu = player_slu_weekly_snapshot.total_slu + EXCLUDED.total_slu` — confirmed in `SluWeeklySnapshotRepository`), so a whole-method retry after a *successful-but-reported-failed* commit re-adds every delta: weekly SLU totals for the affected player/skill/week inflate 2×/N×, silently, with no dedup key. Project-owner decision 2026-08-31 (during `skillars-deferred-85` code review): keep the wide `retryFor` — the observability win is worth it — and close the gap properly in a dedicated story that adds a persistence-level idempotency key to the snapshot write path so a retry is a no-op when the prior attempt actually landed. Needs a key design and a Flyway migration, which is why it is not a review patch. [`src/main/java/com/softropic/skillars/platform/development/repo/SnapshotBatchWriter.java`, `SluWeeklySnapshotRepository.java`, `src/main/java/com/softropic/skillars/platform/development/service/SnapshotPersistenceRetrier.java`, `SluPersistenceRetrier.java`] `[CLOSED by skillars-deferred-86 AC1 — new development.player_slu_weekly_snapshot_applied marker table (V119, PK (session_id, player_id, skill_code, iso_year, iso_week)); SnapshotBatchWriter.writeAll now calls SluWeeklySnapshotRepository.upsertAddIdempotent, a single CTE that inserts the marker ON CONFLICT DO NOTHING RETURNING 1 and applies the additive delta only WHERE EXISTS (SELECT 1 FROM ins). A retry / duplicate replay of the same session's batch is a no-op; two distinct sessions in the same weekly bucket still both apply. upsertAdd deleted outright (SnapshotBatchWriter was its only caller). SnapshotPersistenceRetrier javadoc updated: retry-safety now rests on the marker, not "rolled-back → clean slate".` `The SLU-saveAll half of this bullet was a false premise (skillars-deferred-86 senior-dev review H1, verified against source): PlayerSkillStat has @GeneratedValue(GenerationType.UUID) and no @Version / Persistable, so on an ambiguous-commit retry SimpleJpaRepository.save routes the id-bearing instances through em.merge() — a SELECT-by-id with every non-key column updatable=false, hence no UPDATE, no DataIntegrityViolationException, no duplicate row and no misleading "rows lost" @Recover log. The SLU save was already retry-safe. skillars-deferred-86 AC2 still adds an explicit existsBySessionId(session_id) check-then-act in SluPersistenceRetrier.saveSluWithRetry — as idempotency-intent documentation, a saved merge/SELECT round-trip on retry, and a backstop against a future mutable/@Version column — plus an IT that pins the current merge-clean retry behaviour. Not a bug fix.]`
- **Single-part ETag/MD5 compare can hard-fail a good <8 MB backup upload under bucket encryption.** `pg-backup.sh` / `volume-backup.sh`'s new post-upload verification, in the `*)` (single-part, no `-` in ETag) `case` branch, does `if [ "${REMOTE_ETAG}" != "${LOCAL_MD5}" ]; then err; exit 1`. An S3-compatible ETag equals the raw object MD5 only for a single-part upload to a bucket with no SSE-KMS/SSE-C; the comment only excuses the *multipart* case. While dumps are still under awscli v1's 8 MB `multipart_threshold` (the first months of production, per the comment's own note) and if Hetzner Object Storage applies any server-side encryption or a provider-specific ETag scheme, an intact, already-size-verified upload is declared a failure and the script exits 1 — a false backup failure on good data. Spec-mandated `exit 1` on single-part mismatch (AC4), so left as-is; the `ContentLength == local size` check above it is the reliable leg and is unaffected. Revisit if Hetzner OS is found to enable SSE. [`deploy/backup/pg-backup.sh`, `deploy/backup/volume-backup.sh`] `[CLOSED by skillars-deferred-87 AC1 — the single-part-ETag `*)` branch now WARNs instead of `exit 1`; deferred-85 AC4's spec-mandated `exit 1` is deliberately reversed. The `ContentLength == local size` check is the sole hard gate on upload integrity.]`
- **`head-object` verification read fires immediately after `aws s3 cp` with no retry/backoff.** `pg-backup.sh` / `volume-backup.sh` read the brand-new key once; if Hetzner Object Storage does not guarantee strong read-after-write consistency for new-object PUTs (AWS S3 now does; Hetzner's guarantee is not documented), a transient not-yet-visible object yields an empty `REMOTE_SIZE` and the script `exit 1`s on a good upload (and, per the item above, leaves the local dump behind). Minor hardening — wrap the `head-object` size check in one short retry loop. [`deploy/backup/pg-backup.sh`, `deploy/backup/volume-backup.sh`] `[CLOSED by skillars-deferred-87 AC2 — the head-object ContentLength read is wrapped in a per-script `head_object_content_length()` bounded retry (5 attempts, 3s apart, ~12s); a retrieved-but-wrong size still fails fast, exhaustion still `exit 1`s.]`
- **`first-time-setup.md`'s "If `provision.sh` fails partway through" section over-promises `chown -R` idempotency for the partial-completion case.** It states the one non-idempotent hazard "is already mitigated — `chown_if_needed` skips the recursive `chown` entirely when the directory's current owner already matches." A run killed *mid-`chown -R`* (SIGKILL, OOM, power loss during the exact chown) can leave the top-level directory already owned correctly while its children are only half-changed; the "just re-run" procedure then detects the top-level match and skips, permanently leaving mixed ownership under the data mount. The doc addresses interruption risk but not partial-completion state. Very narrow window. [`docs/deployment/first-time-setup.md`] `[CLOSED by skillars-deferred-87 AC3 — `chown_if_needed` now also does a bounded uid+gid recheck (`find -maxdepth 2`) when the top-level owner matches, catching a run killed mid-`chown -R` with the top level already done; `first-time-setup.md` documents the manual remediation for a mismatch buried deeper.]`
...
- W1: No FK from `player_radar_baselines.player_id` / `coach_radar_preferences.player_id` to `main.player_profiles` — accepted limitation per spec dev notes; consistent with Stories 5.1–5.3 no-FK pattern across `development.*` tables [`V51__radar_display_correlation.sql`] `[PARTIALLY CLOSED by skillars-deferred-77 AC9 — V113 adds ON DELETE CASCADE FK to main.player_profiles(id) for both player_radar_composites AND player_radar_baselines (in-scope decision, not just player_radar_composites as AC9's own text originally hedged). coach_radar_preferences.player_id remains without an FK — out of AC9's scope. [PICKED UP by skillars-deferred-84 AC1: V117 adds the same ON DELETE CASCADE FK to coach_radar_preferences.player_id, mirroring V113's exact defensive-delete-then-constrain pattern. coach_radar_preferences.coach_id remains without an FK — never flagged by any prior review, deliberately left out of AC1's scope as a distinct, not-yet-raised gap.]` `[CLOSED by skillars-deferred-86 AC4 — V118 adds fk_crp_coach_id: coach_radar_preferences.coach_id → marketplace.coach_profiles(id) ON DELETE CASCADE, mirroring V117's exact defensive-delete-then-ADD-CONSTRAINT pattern. No separate index needed — coach_id is the leading column of the (coach_id, player_id) PK. New CoachRadarPreferencesCoachFkIT seeds a dedicated, otherwise-unreferenced coach and asserts the cascade. coach_radar_preferences now has FKs on both PK columns.]`
- W2: Rapid skill-toggle fires a PUT per click — no debounce; last-write-wins for fast toggling; low risk [`PlayerDevelopmentDashboardPage.vue`] `[PICKED UP by skillars-deferred-84 AC3: wraps the store.saveRadarPreferences call in onSkillSelectionChange with a 300ms debounce, mirroring DrillLibraryPage.vue's own established inline useDebounce pattern. The visual toggle itself stays instant — only the network call is debounced.]`
...
- DEF1: `SkillDefinitionRepository` injected directly into `SkillDefinitionResource` (no service layer) — pre-existing architecture; fix at next planned touch of `SkillDefinitionResource` [`SkillDefinitionResource.java:17`] `[PICKED UP by skillars-deferred-84 AC4: this story is that next planned touch — new SkillDefinitionService extracted, SkillDefinitionResource now delegates to it, plus new test coverage (SkillDefinitionServiceTest/SkillDefinitionResourceIT) since neither existed before.]`
...
- W1: Partial snapshot missing if failure occurs between `sluRepository.saveAll` and `snapshotBatchWriter.writeAll` — acknowledged in dev notes; snapshot is eventually-consistent and does not roll back with SLU rows [`SluCalculationService.java:177-187`] `[PARTIALLY ADDRESSED by skillars-deferred-77 AC8 — saveAll itself now retries via SluPersistenceRetrier before SLU rows are considered lost, narrowing this window; writeAll (the snapshot write) still has no retry/compensation of its own, so a failure there specifically remains open. [PICKED UP by skillars-deferred-84 AC2: new SnapshotPersistenceRetrier wraps snapshotBatchWriter.writeAll, mirroring SluPersistenceRetrier's exact @Retryable/@Recover pattern.]]` `[CLOSED by skillars-deferred-86 — after AC1 (per-(session, weekly-bucket) idempotency marker) + AC3 (retriers isolated onto the bounded sluRetryExecutor via SluPersistenceDispatcher), the snapshot write now has retry + structured @Recover + idempotency + an isolated executor. Residual, deliberately not fixed: the SLU save and the snapshot write remain two independent transactions (a shared transaction is explicitly not wanted — the two are independent by design), so a hard crash strictly between them still leaves that week's snapshot un-written until the next session for that player triggers recomputation. Acceptably eventually-consistent, matching the table's stated contract.]`
...
- D3: `SUSPENDED` user in `EMAIL_VERIFIED` state can complete phone OTP — no suspension code exists yet; guard should be updated when suspension story is implemented [CoachRegistrationService.java] `[CLOSED by skillars-deferred-88 AC11 — verifyEmail/verifyPhone in all three registration services now reject a User with locked = true after the user load and before any state mutation. locked is written by admin action / GDPR erasure / user-erased event (NOT any brute-force path — LoginAttemptsService throttles in-memory only). verifyEmail throws EmailTokenException(security.accountLocked, canResend=true) (non-terminal, resumes after unlock); verifyPhone throws OtpVerificationException(security.accountLocked). Per-service locked-user rejection ITs, mutation-verified on verification_status.]`
...
- D5: phone_otp_tokens no partial unique index on active OTPs — multiple valid OTPs possible if service doesn't invalidate old tokens first; verify in Group B [V21__skillars_security_extension.sql] `[CLOSED by skillars-deferred-88 AC10 — V121 adds uq_pot_one_active_per_user partial unique index on main.phone_otp_tokens (user_id) WHERE used = false (+ defensive dedup DELETE). The 4 OTP-insert sites use saveAndFlush so a collision is a synchronous DataIntegrityViolationException → ApiAdvice maps uq_pot_one_active_per_user → 409 security.otpResendInProgress; not caught in-service so no UnexpectedRollbackException. Concurrency IT (6 barrier threads on Parent.resendPhoneOtp) proves exactly one active row survives.]`
...
- W3: SES conditional bean: unrecognized value for `app.ses.enabled` (e.g. `enabled: yes`) leaves `SesEmailService` unwired at startup [SesConfig.java, SesEmailServiceImpl.java] `[CLOSED by skillars-deferred-88 AC9 — new SesEnabledPropertyValidator (EnvironmentPostProcessor via META-INF/spring.factories) rejects any app.ses.enabled value that is not case-insensitively true/false with an IllegalStateException naming the value, before any bean is created. The four @ConditionalOnProperty gates + SesProperties.enabled default are left untouched (already a consistent set). Note the current failure mode is a loud NoSuchBeanDefinitionException, not silent.]`
...
- Empty notification vars cause silent delivery failure — if `GF_ALERT_NOTIFY_EMAIL` or `GF_SLACK_WEBHOOK_URL` are empty (compose defaults), Grafana provisions the contact point but notifications silently fail; intentional spec design tradeoff (`${VAR:-}`) `[CLOSED (both-empty case) by skillars-deferred-85 AC6: provision.sh now hard-fails (exit 1) when .env is present and BOTH GF_ALERT_NOTIFY_EMAIL and GF_SLACK_WEBHOOK_URL are blank, and WARNs when exactly one is set (the other notify-ops receiver still provisions empty) or when GF_ALERT_NOTIFY_EMAIL is set without GF_SMTP_ENABLED=true. Documented in secrets-reference.md + first-time-setup.md. RESIDUAL, still open: a single-channel deployment still provisions one empty notify-ops receiver that silently no-ops — the fuller fix is conditional receiver templating in grafana-alerts.yml, not done here.]` `[RESIDUAL CLOSED by skillars-deferred-88 AC7 — provision.sh render_grafana_contact_points rewrites the # >>> BEGIN provision.sh-managed contactPoints marked region of grafana-alerts.yml from only the channels set in .env, so a single-channel deployment provisions exactly one receiver and no silent no-op. ${GF_*} placeholders kept verbatim (no inlined secrets); committed default keeps both receivers so the file is valid pre-provision. 3-case functional test + YAML-parse verified.]`
...
- No outbound firewall rules — observability containers (Prometheus, Loki, Tempo, Redis) have unrestricted internet egress; security hardening enhancement. `[CLOSED by skillars-deferred-88 AC8 — prometheus/loki/tempo/redis/node_exporter moved onto a new internal:true skillars-observability bridge network (no gateway → no egress); app + grafana join both networks so every existing peer edge still shares a network. Compose-only, no host iptables. docker compose config verified for base + all 3 overlays.]`
...
- `/dev/sdb` hardcoded device path unreliable on multi-volume servers — if Hetzner changes device assignment order the mount silently fails. The doc accuracy fix is a patch (see F2); fixing the script is Story 1.1 territory [deploy/provision.sh:145]. `[CLOSED by skillars-deferred-87 AC4 — provision.sh section 7 resolves the Volume via /dev/disk/by-id/scsi-0HC_Volume_* (readlink -f), falling back to /dev/sdb only when absent; /etc/fstab uses the stable by-id path and a stale /dev/sdb line for the mount point is purged on re-run.]`
...
- D5: No Docker build-layer caching in `pr-build.yml` (only `~/.m2` is cached) — every PR triggers a fully cold image build. [`.github/workflows/pr-build.yml`] `[CLOSED — .github/actions/docker-build/action.yml already carries cache-from: type=gha / cache-to: type=gha,mode=max, used by both ci.yml and pr-build.yml. Found stale during skillars-deferred-87 creation.]`
- D6: The new "defence in depth" doc callout asserts Hetzner's outage behavior ("the cloud firewall remains active... during a Hetzner API outage") as fact with no citation. [`docs/deployment/first-time-setup.md`] `[CLOSED — first-time-setup.md's defence-in-depth callout now explicitly labels the Hetzner-outage behaviour "unconfirmed rather than relying on it". Found stale during skillars-deferred-87 creation.]`
...
- D6: `SoftDeleteIT.concurrentDoubleSoftDelete_exactlyOneSucceeds_oneConflicts` synchronises the *start* of two HTTP round-trips (TCP connect, auth filter chain, controller dispatch) rather than the read-check-write critical section, so it can pass by request serialisation rather than by the lock. The dev recorded a successful mutation verification (reverting to `findById` produced two 204s), so the fix is proven — but the test is not a durable guard, the same class of finding `skillars-deferred-13` and `-15` reviews both raised. Its `SELECT COUNT(*) ... WHERE id = ? AND deleted_at IS NOT NULL` assertion is additionally tautological over a primary key. [`src/test/java/com/softropic/skillars/platform/messaging/api/SoftDeleteIT.java`] `[CLOSED by skillars-deferred-88 AC2 — SoftDeleteIT.concurrentDoubleSoftDelete_… rewritten: an external transaction holds the messaging.messages row lock, stages its own deleted_at, then commits; the concurrent DELETE re-reads deleted_at under the lock and loses with 409 messaging.alreadyDeleted without overwriting it. Mutation-verified by reverting BOTH findByIdForUpdate→findById AND entityManager.refresh(…PESSIMISTIC_WRITE)→NONE together (each alone is a no-op — the @Lock query still emits FOR UPDATE and the save() UPDATE blocks regardless; the read-check on fresh state is what the lock uniquely buys) → DELETE returns 204 not 409, Tests run: 1 Failures: 1. The tautological COUNT(*) assertion is replaced by the losing-caller body check + a stable-deleted_at assertion.]`
...
- **AC5's `concurrency` group doesn't guarantee the newest-pushed commit wins `:latest`.** `build-and-push` still declares `needs: test`, so concurrency-slot readiness tracks `test`-job duration, not push order — a slower older push's job can still complete its `build-and-push` after a faster newer push's, overwriting `:latest` with the older commit. The `concurrency` block does prevent two `build-and-push` runs from interleaving mid-upload, a real improvement; it just doesn't close the ordering race the item was originally filed against. Already surfaced and accepted as a deliberate scope decision in this story's own AC5 and the `deferred-work.md` entry it closed (line 1285) — recorded here only per this review's standard defer-logging convention, not a new finding. A stronger fix (`build-and-push` checking `git merge-base --is-ancestor` against the currently-published `:latest` digest before overwriting) remains out of scope. [`.github/workflows/ci.yml:49-53`] `[CLOSED by skillars-deferred-87 AC6 — build-and-push reads org.opencontainers.image.revision off the published :latest (index-aware — :latest is an OCI index via default provenance) and publishes :latest only when $GITHUB_SHA is a descendant of (or equal to) it, or :latest is absent; any ambiguity — not-ancestor, unknown commit (git rc 128), imagetools/jq error — falls back to publishing the sha- tag only and never aborts the job. The sha- tag is published by every run that executes; only :latest is gated.]`
...
- **Multi-Volume host without `HETZNER_VOLUME_ID` binds to the lexically-first `/dev/disk/by-id/scsi-0HC_Volume_*` symlink and would `mkfs.ext4` it if it carries no filesystem.** AC4 sanctions "first match with a `log` line" for the current single-Volume topology. If a second Volume is ever attached, the operator must export `HETZNER_VOLUME_ID` or the wrong device can be mounted/formatted at `${MOUNT_POINT}` and the wrong `by-id` path written to `/etc/fstab`. [`deploy/provision.sh:196-202`] `[CLOSED by skillars-deferred-88 AC5 — device resolution enumerates the scsi-0HC_Volume_* symlinks once; err + exit 1 on (>1 symlink AND HETZNER_VOLUME_ID unset) AND on (>1 symlink AND HETZNER_VOLUME_ID set-but-unresolvable). Single/zero-Volume + unresolvable id keeps warn-then-fall-back. 7-case hand-trace verified.]`
- **Staging `rsync` duplicates the pre-Volume tree on the same root filesystem with no free-space pre-check.** The migration exists because the root disk is filling; `rsync -aHAX "${MOUNT_POINT}/" "${STAGING}/"` doubles the tree on that same disk. ENOSPC → `set -euo pipefail` abort *before* `mount` → every re-run loops with the root disk fuller than before. Pre-Volume payload is expected small per AC5 (certs + a short window of LGTM/Redis data), but a large pre-Volume observability retention would bite. A `df`-based guard before staging would close it. [`deploy/provision.sh:297-298`] `[CLOSED by skillars-deferred-88 AC5 — du -sk / df -Pk free-space guard (2x headroom) before the staging rsync, armed-migration only, || true-guarded, fail-open on an unreadable du/df, err + exit 1 on payload_kb*2 > avail_kb.]`
- **No `flock` anywhere in `provision.sh`.** Two overlapping runs (two operators, or manual + automation) can `rm -rf "${STAGING}"` while the other is mid-`rsync "${STAGING}/" "${MOUNT_POINT}/"`, or double `mount` / `mkfs.ext4` the same device. Pre-existing — the script has never had a lock; concurrent runs are not a supported scenario. [`deploy/provision.sh` section 7] `[CLOSED by skillars-deferred-88 AC3 — new section 0 acquires a whole-script exclusive flock --nonblock on /var/lock/skillars-provision.lock via a _PROVISION_LOCKED re-exec; a second concurrent run fails fast with a rephrased refusal message. File-invocation only (not curl|bash), which is every documented path.]`
- **A `build-and-push` run cancelled while *pending* in the `build-and-push-${{ github.ref }}` concurrency group publishes no tags at all — not even `sha-<short>`.** GitHub keeps only one run pending per group and cancels a previously-pending one when a newer queues, so in a ≥3-push burst the middle commit's image is never built and `docs/deployment/rollback.md` has no rollback target for it until its CI is manually re-run. `cancel-in-progress: false` protects only the *running* run. Acknowledged in the shipped `ci.yml` comment and the story's M2 caveat; AC6 neither introduces nor fixes it. [`.github/workflows/ci.yml:52-54`]
- **`sed -i` stale-fstab purge matches a commented-out `# … /opt/skillars/data …` line.** `grep -qE "[[:space:]]${MOUNT_POINT}[[:space:]]"` on the `grep -vFx "${FSTAB_ENTRY}"` output, then `sed -i "\#[[:space:]]${MOUNT_POINT}[[:space:]]#d"`, will also delete an operator's deliberately commented alternate entry — but only when an *active* non-canonical line for the same mount point also exists (that is what arms the `if`). Low impact. [`deploy/provision.sh:320-321`] `[CLOSED by skillars-deferred-88 AC4 — one shared regex ^[[:space:]]*[^#[:space:]][^[:space:]]*[[:space:]]+${MOUNT_POINT}[[:space:]] in both the guard grep and the sed address (sed delimiter , since # is in the bracket expr); a commented-out alternate is never matched. Functional 4-case hand-trace verified.]`
- **`chown_if_needed`'s `find -maxdepth 2` walks grandchildren, not just the "immediate children" the comment describes, and only `-quit`s on a mismatch.** On a clean idempotent re-run it stat-walks the directory plus two levels of each observability dir (`loki/chunks/*`, tempo blocks, prometheus WAL) with no early exit. Matches the spec's literal `-maxdepth 2`; revisit if provision runtime regresses on a Volume with real retention. [`deploy/provision.sh:43`]
...
- **`provision.sh` edits `/etc/fstab` in place with `sed -i` and no `.bak`.** The delete pattern is whitespace-anchored to the exact constant mount point and only runs when a non-canonical active line exists, so the blast radius is small — but a boot-critical file is modified with no recovery artifact. A `cp /etc/fstab /etc/fstab.bak` before the `sed` would be cheap insurance; it would be a new convention for this script. [`deploy/provision.sh:321`] `[CLOSED by skillars-deferred-88 AC4 — timestamped cp -p /etc/fstab /etc/fstab.bak.<ts> INSIDE the if-block, immediately before the sed (only on the run that mutates it).]`
...
- **`${DEPLOY_ROOT}` / `${MOUNT_POINT}` interpolated unescaped into the `/etc/fstab` `grep -E` / `sed -E` pattern.** `_fstab_stale_re="^[[:space:]]*[^#[:space:]][^[:space:]]*[[:space:]]+${MOUNT_POINT}[[:space:]]"` is used verbatim in the guard `grep -qE` and the `sed -i -E "\\,…,d"` address. If `DEPLOY_ROOT` ever contains a regex metacharacter (`.`, `+`, `[`), the address matches unintended `/etc/fstab` lines and `sed -i` deletes them. Safe only because `DEPLOY_ROOT` is currently the metacharacter-free constant `/opt/skillars`; no guard enforces that. AC4 refined the regex body but the unescaped-variable pattern predates this story (the pre-88 code did `sed -i "\#[[:space:]]${MOUNT_POINT}[[:space:]]#d"`). [`deploy/provision.sh` fstab-maintenance block]
...
- **The fstab add/purge block has no `[ -n "${VOLUME_LINK}" ]` precondition.** If zero `scsi-0HC_Volume_*` by-id symlinks resolve but `/dev/sdb` is a block device, `FSTAB_ENTRY` becomes `" /opt/skillars/data ext4 defaults,nofail 0 2"` (empty device field); a valid pre-existing by-id line for the mount point matches `_fstab_stale_re`, is backed up + `sed`-deleted, and the device-less entry is appended — the next `mount -a` / reboot fails on the malformed line. Pre-existing from skillars-deferred-87 AC4; narrow reachability on Hetzner (the by-id symlink is stable while a Volume is attached). Guard: wrap the fstab add/purge block in `if [ -n "${VOLUME_LINK}" ]; then … fi`. [`deploy/provision.sh:~273, ~419-427`]
