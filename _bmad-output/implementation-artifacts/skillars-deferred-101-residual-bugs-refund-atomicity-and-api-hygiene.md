...
_bmad-code-review 2026-09-08 (all three lenses — Blind Hunter, Edge Case Hunter, Acceptance Auditor — run inline against `master...HEAD`). 0 decision-needed, 11 patch, 0 defer, 3 dismissed._

**Resolution status (2026-09-08, second review pass): all 11 patches resolved.** Commits `207d92ec`,
`703a8fca`, `97ba657f` fixed 7 (findings #1, #2, #3-guards, #5, #10, #11, javadoc). The second
review pass found #4, #6, #8, #9 not done and #7 mis-assessed; those are now resolved below.

### Patch — CI-breaking (must fix before merge)

- [x] [Review][Patch] `CancellationRefundMatrixTest` — the 6 broken `verify(refundOutboxSupport).enqueueBookingRefund(...)` assertions on direct `CancellationRefundService` calls were removed; the enqueue-condition matrix is relocated to the new `RefundEnqueueListenerTest` (11 mutation-sensitive `@InjectMocks` unit tests over all 6 guard conditions). Commit `207d92ec`. [CancellationRefundMatrixTest.java, RefundEnqueueListenerTest.java]
- [x] [Review][Patch] `PackCancellationRefundIT` — the credit-path scenarios now publish through `eventPublisher.publishEvent(event)` inside `transactionTemplate.execute(...)`, so both the `BEFORE_COMMIT` `RefundEnqueueListener` and the `AFTER_COMMIT` `CancellationRefundService` listeners fire on the real event path. Commit `97ba657f`. [PackCancellationRefundIT.java:97-140]

### Patch — spec (Test) criteria

- [x] [Review][Patch] **AC4 atomicity coverage** — happy-path same-tx enqueue is now covered end-to-end by `PackCancellationRefundIT.coachCancels_packExpired_creditWritten_sessionNotRestored` (event published in a tx → `RefundEnqueueListener` BEFORE_COMMIT enqueues → `countLedgerEntries == 1` after commit). Guard conditions (all 6) covered by `RefundEnqueueListenerTest`. The BEFORE_COMMIT-rollback-⇒-zero-outbox case remains proven by construction (Spring rolls the publishing tx back if a `BEFORE_COMMIT` listener throws; `enqueueBookingRefund` is `@Transactional(MANDATORY)` so it joins that tx) rather than by a dedicated fault-injection IT — same rationale the codebase applies to the 23 `deferred-92` AC4 email listeners, which shipped without a per-listener rollback IT. [RefundEnqueueListenerTest.java, PackCancellationRefundIT.java]
- [x] [Review][Patch] **AC3 orphan-tracking IT added** — `VideoRetryUploadIT.retryUpload_tracksPriorProviderAssetForSweeper`: seeds a FAILED video with `providerAssetId="old-bunny-guid"` and **no** tracking row, calls `retryUpload` (→ `"new-bunny-guid"`), asserts (a) `Video.providerAssetId == "new-bunny-guid"`, (b) a `pending_provider_asset` row now exists for `"old-bunny-guid"` (mutation: revert AC3's `record(priorAssetId, …)` → fails), (c) the retry's own tracking row for `"new-bunny-guid"` was cleaned up. The sweeper's delete + `ORPHANED_ASSET` incident behaviour is already covered by `OrphanedProviderAssetSweepIT`. [src/test/java/com/softropic/skillars/platform/video/service/VideoRetryUploadIT.java]
- [x] [Review][Patch] **AC10(a) boundary test is now mutation-sensitive** — `isInValidRange_boundaryValues` now stubs `sluTargetRepository.findDistinctPlayerIds()` and asserts `verify(sluTargetRepository).findMaxTargetPerSkill(...)` for in-range thresholds (`0.0001`, `0.5`, `0.9999`) and `verify(..., never())` for the out-of-range boundaries (`0`, `1.0`). Widening `isInValidRange`'s bounds flips a case. Commit `97ba657f`. [NeglectedSkillDetectionServiceTest.java:208-231]
- [x] [Review][Patch] **AC10(b) real-`401` IT added** — `VideoApprovalResourceIT.listPendingApprovals_principalWithoutValidUserId_returns401NotServerError`: a `@WithMockUser(roles = "PARENT")` request (passes `@PreAuthorize`) with `securityUtil.requireCurrentUserId()` stubbed to throw `InsufficientAuthenticationException` from inside the controller → asserts HTTP `401`, not `500`. Exercises the real advice/entry-point mapping through MockMvc (`VideoApiAdvice` has no handler for it, so it propagates to `ExceptionTranslationFilter` → 401). Directly closes `skillars-deferred-1` D1's "a test asserting the actual 401 response when it throws". [src/test/java/com/softropic/skillars/platform/video/api/VideoApprovalResourceIT.java]
- [x] [Review][Patch] **AC11 verified safe — no revert needed.** The concern (concurrent `markAllUsedByUserId` version bump vs `AuthService.refresh`'s managed `saveAndFlush` → unhandled `ObjectOptimisticLockingFailureException` → 500) does **not** hold: `AuthService.java:167-173` already wraps `refreshTokenRepository.saveAndFlush(token)` in `catch (ObjectOptimisticLockingFailureException ex) { clearAuthCookies(res); throw new BadCredentialsException("Concurrent refresh detected — please sign in again"); }` → a clean **401**, not a 500. The AC11 bump actually **closes a latent hole**: pre-bump, a GDPR bulk-revoke (`markAllUsedByUserId`, no version bump) racing an in-flight `refresh` let the `refresh` succeed and re-issue cookies for a just-erased user; post-bump it now hits that OL catch → 401. `logout`'s `save(t)` (`:214-215`) has a marginal OL-vs-`markAllUsedByUserId` race, but `logout` already had the equivalent race against `refresh`'s rotation (which has always bumped the version), so AC11 does not change `logout`'s risk profile. Regression: `AuthResourceIT.refresh_validUnusedToken_rotatesTokenAndReturns200` covers the happy path with the bump; `NativeModifyingVersionAuditTest` (map now empty) enforces the compliant SQL. No fallback taken; javadoc + audit-test comment updated to reflect the reasoning. [src/main/java/com/softropic/skillars/platform/security/service/AuthService.java:167-173, RefreshTokenRepository.java]
- [x] [Review][Patch] **AC12 + AC13 ledger prune done for real.** `deferred-work.md` now physically removes: the `skillars-3-1` weekStart bullet + its emptied header (the `(closed by skillars-deferred-101)` placeholder is gone — delete-outright, no tag); the `deploy-2-2` + `deploy-1-3` rows in the 2026-09-04 audit table (already removed in `703a8fca`) plus the `deploy-1-3` prometheus *bullet*; all six migration `ACCESS EXCLUSIVE`/unbatched-DML bullets across V60/V94/V97/V98/V117 (headers for `skillars-deferred-33`/`-40`/`-84` emptied and removed) → consolidated into `docs/deployment/migration-conventions.md#known-lock-unsafe-applied-migrations`; `skillars-10-2` D1, `skillars-7-2` Group 3 D11, `deploy-3-4` DROP-DATABASE, `skillars-4-5` R2 W5, `skillars-deferred-1` D1/D2, and the four `skillars-deferred-100` code-review bullets closed by AC1/AC2/AC3/AC11. The `## Last audit: 2026-09-08` block is rewritten to match exactly what was removed and carries an explicit line-for-line **Reconstruction check** statement; no `[DECIDED]`/`[DISMISSED]`/`[PICKED UP]` bullet was touched. [_bmad-output/implementation-artifacts/deferred-work.md]
- [x] [Review][Patch] **AC4 ledger clause done** — `skillars-10-2` D1 removed (above); the `deferred-91`/`-92` "AFTER_COMMIT reliability catalogue" bullet's refund-path clause now records that the enqueue moved off `CancellationRefundService`'s `AFTER_COMMIT` listeners into `RefundEnqueueListener` (`BEFORE_COMMIT`, `MANDATORY`), atomic with the `CANCELLED` write, with the sibling `AFTER_COMMIT` listeners keeping pack-restore / history / strike. [_bmad-output/implementation-artifacts/deferred-work.md]

### Patch — code quality

- [x] [Review][Patch] **AC3 comment corrected** — the `VideoService.retryUpload` comment now states `pendingProviderAssetTracker.record(...)` is `REQUIRES_NEW` (commits in its own tx) and that a rolled-back pointer-swap leaves a harmless stale tracking row the sweeper's `findByProviderAssetId` guard drops — it no longer claims atomicity. Commit `207d92ec`. [VideoService.java:197-201]
- [x] [Review][Patch] **Dead dependency removed** — `CancellationRefundService.refundOutboxSupport` field deleted (same-package type, no import to drop). Commit `207d92ec`. [CancellationRefundService.java:33]
- [x] [Review][Patch] **`RefundOutboxSupport.enqueueBookingRefund` javadoc strengthened** — now states the `MANDATORY` propagation requires an active tx and the method must be called from a `BEFORE_COMMIT` `@TransactionalEventListener` (`RefundEnqueueListener`). Commit `207d92ec`. [RefundOutboxSupport.java:40-55]

### Dismissed (noise / verified non-issues)

- AC3 "retry-of-a-retry" duplicate `record(priorAssetId)` — non-issue: `PendingProviderAssetTracker.record` uses `INSERT … ON CONFLICT (provider_asset_id) DO NOTHING` (`repository.insertIfAbsent`), a genuine no-op on a duplicate.
- `AdminVideoService` AC2 `incident == null` branch — verified safe: `incidentDto = incident == null ? null : new ReconcileIncidentDto(...)` already handles it; a no-op reconcile correctly returns `incident: null`.
- AC2 `log.info` moved inside the `transactionTemplate.execute` lambda in `ReconciliationWorkerScheduler` — cosmetic; consistent with `recordIncident` in the same lambda.
...
**Session 3 (2026-09-08, code-review remediation — second pass):** first patch round (`207d92ec`,
`703a8fca`, `97ba657f`) resolved 7 of 11 review findings; a follow-up review found 4 not done and
1 mis-assessed. Closed here:

- **AC3 orphan-tracking IT (finding #4):** added `VideoRetryUploadIT.retryUpload_tracksPriorProviderAssetForSweeper`
  — mutation-sensitive on AC3's `pendingProviderAssetTracker.record(priorAssetId, …)` call.
- **AC10(b) real-401 IT (finding #6):** added `VideoApprovalResourceIT.listPendingApprovals_principalWithoutValidUserId_returns401NotServerError`
  — authenticated PARENT principal, `securityUtil.requireCurrentUserId()` stubbed to throw
  `InsufficientAuthenticationException` from inside the controller → asserts 401 (not 500) through
  the real MockMvc/advice/entry-point path. Closes `skillars-deferred-1` D1's second sub-gap.
- **AC11 verification (finding #7):** verified by code read — `AuthService.java:167-173` already
  catches `ObjectOptimisticLockingFailureException` from `refreshTokenRepository.saveAndFlush(token)`
  and maps it to a clean `BadCredentialsException` (401). The `version = version + 1` bump is safe
  and additionally closes a latent GDPR-bulk-revoke-vs-in-flight-refresh session-resurrection hole.
  `logout`'s `save(t)` OL-vs-`markAllUsedByUserId` race is marginal and unchanged in character
  (it already raced `refresh`'s always-versioned rotation). No revert; no fallback needed.
  Regression: `AuthResourceIT.refresh_validUnusedToken_rotatesTokenAndReturns200` +
  `NativeModifyingVersionAuditTest` (compliant set).
- **AC12/AC13 ledger prune (findings #8, #9):** `deferred-work.md` now physically deletes all the
  bullets the audit block claimed — the `skillars-3-1` weekStart bullet + its emptied header (the
  `(closed by …)` placeholder removed), the `deploy-1-3` prometheus bullet, all 6 migration
  `ACCESS EXCLUSIVE`/unbatched-DML bullets (V60/V94/V97/V98/V117; three now-empty headers removed),
  `skillars-10-2` D1, `skillars-7-2` Group 3 D11, `deploy-3-4` DROP-DATABASE, `skillars-4-5` R2 W5,
  `skillars-deferred-1` D1/D2, and the four `skillars-deferred-100` code-review bullets closed by
  AC1/AC2/AC3/AC11. Audit block rewritten to match reality + explicit line-for-line reconstruction
  check. `deferred-91`/`-92` AFTER_COMMIT catalogue bullet updated for the AC4 split.

**All 14 Acceptance Criteria implemented; all 11 code-review findings resolved.** GitHub CI is the
...
| `_bmad-output/implementation-artifacts/deferred-work.md` | 12,13 + review | ✅ Done | Physically deleted all AC-closed + 6 migration-lock bullets (4 empty headers removed); audit block rewritten + reconstruction check; AFTER_COMMIT catalogue updated for AC4 split |
| `src/test/java/.../payment/service/CancellationRefundMatrixTest.java` | 4 + review | ✅ Done | Removed 6 stale `verify(refundOutboxSupport)` calls (enqueue moved to RefundEnqueueListener) |
| `src/test/java/.../payment/service/RefundEnqueueListenerTest.java` | 4 + review | ✅ Done | NEW: 11 mutation-sensitive unit tests over all 6 enqueue guard conditions |
| `src/test/java/.../payment/service/PackCancellationRefundIT.java` | 4 + review | ✅ Done | Credit-path scenarios publish via ApplicationEventPublisher (real BEFORE_COMMIT + AFTER_COMMIT path) |
| `src/test/java/.../video/service/VideoRetryUploadIT.java` | 3 + review | ✅ Done | NEW test: retryUpload tracks prior provider asset for the orphan sweeper (mutation-sensitive) |
| `src/test/java/.../video/api/VideoApprovalResourceIT.java` | 10 + review | ✅ Done | NEW test: requireCurrentUserId() throw → HTTP 401 (not 500) — closes skillars-deferred-1 D1 |
