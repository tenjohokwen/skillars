...
_bmad-code-review 2026-09-08 (all three lenses — Blind Hunter, Edge Case Hunter, Acceptance Auditor — run inline against `master...HEAD` = 6 commits). 0 decision-needed, 11 patch, 0 defer, 3 dismissed._

### Patch — CI-breaking (must fix before merge)

- [ ] [Review][Patch] `CancellationRefundMatrixTest` — 6 tests assert `verify(refundOutboxSupport).enqueueBookingRefund(...)` on **direct** `CancellationRefundService.onBookingCancelledBy*` calls, but AC4 moved every enqueue to `RefundEnqueueListener`; those `verify` calls now fail "Wanted but not invoked". Not updated in the diff, and not in the dev's "54 targeted tests" local run. Relocate the enqueue-condition matrix to a new `RefundEnqueueListenerTest` (same `@InjectMocks` unit style on `RefundEnqueueListener`) and drop/adjust the matrix test's `refundOutboxSupport` verifies. [src/test/java/com/softropic/skillars/platform/payment/service/CancellationRefundMatrixTest.java:45,92,105,141,153,179]
- [ ] [Review][Patch] `PackCancellationRefundIT.coachCancels_packExpired_creditWritten_sessionNotRestored` (and any other credit-path scenario in that IT) calls `cancellationRefundService.onBookingCancelledByCoach(event)` directly and asserts `countLedgerEntries(bookingId) == 1`. A direct method call no longer triggers the `BEFORE_COMMIT` `RefundEnqueueListener`, so no outbox row → no ledger entry → assertion fails. Drive the scenario through `ApplicationEventPublisher` (real event path) so the listener fires, or assert on the outbox row. [src/test/java/com/softropic/skillars/platform/payment/service/PackCancellationRefundIT.java:95-110]

### Patch — spec (Test) criteria unmet, marked `[x]` anyway

- [ ] [Review][Patch] **AC4 atomicity IT missing** — the highest-risk AC (money path, new BEFORE_COMMIT + `MANDATORY` pattern, new listener class) ships with **zero** new test coverage. Spec required an IT over `onBookingCancelledByCoach` + `onBookingCancelledByAdmin`: (1) happy-path booking→CANCELLED + exactly one `BOOKING_REFUND` outbox row in the same tx; (2) business-tx rollback ⇒ zero outbox rows + no strike/history; (3) strike still issues on the happy path. [src/main/java/com/softropic/skillars/platform/payment/service/RefundEnqueueListener.java]
- [ ] [Review][Patch] **AC3 orphan-tracking IT missing** — spec required: video→FAILED with `providerAssetId=A` (no tracking row), `retryUpload`→B, assert a `pending_provider_asset` row for A exists, assert `sweepOrphanedProviderAssets` then calls `deleteAsset("A")` + writes an `ORPHANED_ASSET` incident. Not implemented; Task 3 marked `[x]`. [src/main/java/com/softropic/skillars/platform/video/service/VideoService.java:195]
- [ ] [Review][Patch] **AC10(a) test asserts nothing** — `isInValidRange_boundaryValues` is a `@ParameterizedTest` that only calls `detectNeglectedSkills()`; it is not mutation-sensitive (widening `isInValidRange`'s bounds would not fail it). Assert the documented per-branch behaviour — e.g. `snapshotRepository` invoked for a valid threshold, `never()` invoked for `0` / `1.0`. [src/test/java/com/softropic/skillars/platform/development/service/NeglectedSkillDetectionServiceTest.java:209-227]
- [ ] [Review][Patch] **AC10(b) real-`401` IT entirely missing** — spec required one IT hitting a `requireCurrentUserId` Resource unauthenticated / with a principal lacking a business id, asserting `401` + the error body. Not implemented; Task 10 sub-item marked `[x]`. [—]
- [ ] [Review][Patch] **AC11 reversed a documented deliberate decision without the mandated regression check** — the removed javadoc warned that `SET … version = version + 1` turns benign convergent `logout`/`refresh` races into `ObjectOptimisticLockingFailureException`s that `AuthService.logout`/`refresh` do not handle (→ 500 instead of a clean 401). Spec required running `AuthServiceIT` / `AuthResourceIT` / the GDPR erasure IT and, on any OL failure, taking the fallback (revert the bump; keep the allow-list entry + add a guard test that fails the build if a new `refresh_tokens` writer sets any column other than `used`). Only `NativeModifyingVersionAuditTest` was run. Run the auth + GDPR ITs, record results in the Dev Agent Record, apply the fallback if any OL failure surfaces. [src/main/java/com/softropic/skillars/platform/security/repo/RefreshTokenRepository.java:32]
- [ ] [Review][Patch] **AC12 + AC13 ledger prune never happened** — `deferred-work.md` diff is `+26 / −0`. The appended `## Last audit: 2026-09-08` block states the six migration bullets, the `skillars-3-1` weekStart bullet, the `deploy-2-2` Fail-workflow bullet, and "all AC-closed bullets" were "deleted this audit" — but every one of them is still in the file, now contradicted by the new block. Actually perform the deletions: the six V60/V94/V97/V98/V117 bullets → one pointer line to `migration-conventions.md#known-lock-unsafe-applied-migrations`; remove `skillars-3-1` weekStart bullet + its header, the `deploy-2-2` row, and the AC-closed bullets (`skillars-10-2` D1, Group 3 D11, `deploy-3-4` DROP-DATABASE, `deploy-1-3` prometheus, `skillars-4-5` R2 W5, `skillars-deferred-1` D1/D2, and the `deferred-100` code-review bullets for `reconcileToReady` / `RefreshTokenRepository.markAllUsedByUserId`). Then reconcile the audit-block wording with what was actually removed and add the line-for-line reconstruction-check statement the AC requires. [_bmad-output/implementation-artifacts/deferred-work.md]
- [ ] [Review][Patch] **AC4 ledger clause not done** — `skillars-10-2` D1 not deleted (covered above), and the `deferred-91`/`-92` "AFTER_COMMIT reliability catalogue" bullet's `CancellationRefundService` mention was not updated to record that the `BOOKING_REFUND` enqueue is now a `BEFORE_COMMIT` sibling listener. [_bmad-output/implementation-artifacts/deferred-work.md]

### Patch — code quality

- [ ] [Review][Patch] **AC3 comment falsely claims atomicity** — `pendingProviderAssetTracker.record(...)` is `@Transactional(propagation = REQUIRES_NEW)`, so it suspends the caller and commits in its own tx; the new comment "Must happen in this tx with the Video write so the pointer-swap and tracking row commit atomically" is wrong. The end state is still safe (a rolled-back pointer-swap leaves `Video.providerAssetId` pointing at the prior asset, and `ReconciliationWorkerScheduler.processPendingProviderAsset`'s `findByProviderAssetId` guard then just drops the stale tracking row rather than deleting a referenced asset). Fix the comment to state that, rather than claiming atomicity. [src/main/java/com/softropic/skillars/platform/video/service/VideoService.java:197-201]
- [ ] [Review][Patch] **Dead injected dependency** — `CancellationRefundService.refundOutboxSupport` field + its import are now unused (all six call sites moved to `RefundEnqueueListener`). Remove them. [src/main/java/com/softropic/skillars/platform/payment/service/CancellationRefundService.java:33]
- [ ] [Review][Patch] **`RefundOutboxSupport.enqueueBookingRefund` `MANDATORY` contract is now load-bearing** — a future non-transactional caller gets `IllegalTransactionStateException`. No live break (only the six `BEFORE_COMMIT` listener sites call it today). Strengthen the class/method javadoc to state it must be invoked from a `BEFORE_COMMIT` `@TransactionalEventListener` (or otherwise within the producing transaction). [src/main/java/com/softropic/skillars/platform/payment/service/RefundOutboxSupport.java:40-45]
