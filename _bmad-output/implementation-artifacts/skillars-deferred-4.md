...
**Modified Files:**
- `pom.xml`
- `src/main/java/com/softropic/skillars/platform/notification/config/AsyncConfig.java` (`@EnableScheduling` → `@EnableSchedulerLock`; later given an explicit `order` to fix D3)
- `src/main/java/com/softropic/skillars/infrastructure/config/ShedLockConfig.java` (D2: log + `scheduler.lock.skipped` metric when a lock is already held)
- `src/main/java/com/softropic/skillars/platform/development/service/NeglectedSkillDetectionService.java` (D1: runtime warning past 80% of `lockAtMostFor`)
...
- `src/main/java/com/softropic/skillars/platform/video/service/QuotaReservationTimeoutService.java` (D1: 8-minute self-terminating drain budget)
...
- `src/test/java/com/softropic/skillars/infrastructure/config/ShedLockConfigIT.java` (D2: new `lockProvider_skipsAndRecordsMetricWhenAlreadyHeld` test)
...
- [x] [Review][Defer] `lockAtMostFor` timeouts on `QuotaReservationTimeoutService` and `NeglectedSkillDetectionService` are not validated against their unbounded work loops (an un-chunked `do/while` batch drain and an un-chunked per-player loop, respectively) — under data growth, ShedLock could force-expire the lock mid-run and let a second instance start an overlapping execution, reopening the exact race AC1 exists to close. [src/main/java/com/softropic/skillars/platform/video/service/QuotaReservationTimeoutService.java:28, src/main/java/com/softropic/skillars/platform/development/service/NeglectedSkillDetectionService.java:58] — resolved differently per job, driven by retry cadence: `QuotaReservationTimeoutService` fires every 60s (fixedDelay) and its batches are already idempotent (SKIP LOCKED), so its drain loop now self-terminates on an 8-minute wall-clock budget (safety margin under `lockAtMostFor=PT10M`) and logs a warning, leaving the remainder for the next firing. `NeglectedSkillDetectionService` only fires weekly, so a hard mid-run cutoff would strand unprocessed players for a week — instead it now logs a loud warning once the per-player loop crosses 24 minutes (80% of the `PT30M` budget), giving ops time to raise `lockAtMostFor` before growth actually breaches it; per-player work was already safe to double-run via the `idx_neglected_flags_open` partial unique index (V49).
- [x] [Review][Defer] No log or metric is emitted when `@SchedulerLock` skips a run because another instance already holds the lock — indistinguishable in production from a job silently failing to run due to a bug. [src/main/java/com/softropic/skillars/platform/notification/config/AsyncConfig.java] — resolved: `ShedLockConfig.lockProvider()` now wraps the `JdbcTemplateLockProvider` delegate; whenever `lock()` returns `Optional.empty()` (ShedLock's sole signal that another instance holds the lock), it logs at INFO and increments a `scheduler.lock.skipped` Micrometer counter tagged by `lock_name`. Covers every `@SchedulerLock`-annotated method app-wide, not just the ones named in this finding. Verified with `ShedLockConfigIT.lockProvider_skipsAndRecordsMetricWhenAlreadyHeld()`.
- [x] [Review][Defer] `@SchedulerLock` and `@Transactional` are stacked on the same method with no explicit `@Order`, so their AOP advisor nesting order (lock vs. transaction boundary) is unspecified — plausible but unconfirmed risk that the distributed lock could release before the DB transaction commits. [src/main/java/com/softropic/skillars/platform/booking/service/BookingExpiryScheduler.java:40, src/main/java/com/softropic/skillars/platform/booking/service/BookingReminderScheduler.java, src/main/java/com/softropic/skillars/platform/video/service/BandwidthResetService.java] — resolved: confirmed the risk is real, not just plausible. `@EnableSchedulerLock`'s `interceptMode` defaults to `PROXY_METHOD` (verified by decompiling the annotation's `AnnotationDefault` attribute in `shedlock-spring-5.13.0.jar`), which registers a genuine `MethodProxyScheduledLockAdvisor` on the same AOP proxy chain as `@Transactional`. Both advisors defaulted to `Ordered.LOWEST_PRECEDENCE`, so their relative nesting depended on unspecified advisor-registration order. Fixed centrally in `AsyncConfig`: `@EnableSchedulerLock(..., order = Ordered.LOWEST_PRECEDENCE - 100)` forces the lock advisor outermost for every `@SchedulerLock` method app-wide, guaranteeing `proceed()` — and therefore the full transaction commit/rollback — always completes before the lock is released.
