...
| `PLATFORM_REGISTRATION_VERIFICATION_SECRET` | String | dev placeholder; **empty under `prod`/`uat` — `RegistrationVerificationTokenService` aborts startup if unset** | `openssl rand -base64 32`. HMAC key for the phone-verification handle returned by `/verify-email` and required by `/verify-phone` and `/resend-otp`. Rotating it only invalidates registration handles issued in the last 24h |
