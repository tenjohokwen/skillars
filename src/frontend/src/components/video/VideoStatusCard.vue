<template>
  <q-card
    v-if="statusConfig"
    class="video-status-card glass-card"
    role="region"
    :aria-label="t('video.statusCard.title')"
    aria-live="polite"
  >
    <q-card-section class="row items-center q-gutter-sm">
      ...
      <div>
        ...
        <div class="text-subtitle2" :aria-atomic="true">
          {{ statusLabel }}
        </div>
      </div>
      <q-space />
...
const statusLabel = computed(() =>
  t(`video.status.${currentStatus.value}`, t('video.status.unknown')),
)
...
watch(
  [() => props.videoId, () => props.initialStatus],
  ([vid, status]) => {
