      ...
      <div class="glass-card--static auth-card">
        ...
        <template v-else-if="hasError">
          ...
          <div v-if="canResend" class="text-center q-mt-lg">
            <q-btn
              flat no-caps
              :label="t('auth.player.resendEmail')"
...
onMounted(async () => {
  ...
  if (emailFromUrl) {
    sessionStorage.setItem('pendingPlayerEmail', emailFromUrl)
  }
...
async function handleResendFromVerify() {
  router.push({ path: '/player/email-pending', query: { email: sessionStorage.getItem('pendingPlayerEmail') || undefined } })
}
</script>
