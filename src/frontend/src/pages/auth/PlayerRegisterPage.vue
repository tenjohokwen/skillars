...
const mustBeAdult = computed(() => errorKey.value === 'security.playerMustBeAdult')

const isAdult = computed(() => {
  if (!form.value.dateOfBirth) return true // don't nag before they've entered anything
  return calculateAge(form.value.dateOfBirth) >= 18
})
...
async function handleRegister() {
  ...
  try {
    ...
    await playerRegistrationApi.register({
    ...
    sessionStorage.setItem('pendingPlayerEmail', form.value.email)
    router.push('/player/email-pending')
  } catch (err) {
    setError(err)
  } finally {
