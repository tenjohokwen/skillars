...
onMounted(async () => {
  ...
  try {
    ...
    sessionStorage.setItem('coachVerificationToken', verificationToken)
    router.push({ path: '/coach/verify-phone', replace: true })
  } catch (err) {
    isVerifying.value = false
...
async function handleResendFromVerify() {
  router.push({
    ...
    query: { email: sessionStorage.getItem('pendingCoachEmail') || undefined },
  })
}
</script>
