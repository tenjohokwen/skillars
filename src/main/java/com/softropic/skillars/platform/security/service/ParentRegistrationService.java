...
public class ParentRegistrationService {
    ...
    @RateLimited(key = "parent_register", capacity = 3, duration = 60)
    public void registerParent(ParentRegistrationRequest req) {
        if (userRepository.findOneByEmail(req.email()).isPresent()) {
            throw new ParentRegistrationException("security.emailInUse", "Email already registered");
        }
        ...
        Authority parentAuthority = authorityRepository.findOneByName(SecurityConstants.ROLE_PARENT)
            .orElseThrow(() -> new IllegalStateException("ROLE_PARENT authority not found"));
        ...
        user.setAuthorities(Set.of(parentAuthority));
        ...
        try {
            ...
            if (msg.contains("phone")) {
                throw new ParentRegistrationException("security.phoneInUse", "Phone number already registered");
            }
            throw new ParentRegistrationException("security.emailInUse", "Email already registered");
        }
    ...
    public void verifyPhone(Long userId, String otp) {
        if (!rateLimitingService.tryConsume(String.valueOf(userId), "parent_otp_verify", 10, 10, TimeUnit.MINUTES)) {
    ...
    public void resendVerificationEmail(String email) {
        userRepository.findOneByEmail(email).ifPresent(user -> {
            ...
            if (user.getVerificationStatus() == null ||
                ...
                sendVerificationEmail(user);
            } else if (user.getVerificationStatus() == SkillarsVerificationStatus.EMAIL_VERIFIED) {
                ...
                verificationTokenService.issuePhoneVerificationToken(user.getId(), "PARENT");
            }
        });
    }
    ...
    private void sendOtpEmail(User user, String otp) {
        publisher.publishEvent(new ParentOtpEmailEvent(
